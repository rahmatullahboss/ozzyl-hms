...
import { centralRoutePermissionShadowFromEnv } from './lib/route-permissions';
...
import { logServerError, shouldLogServerErrorResponse, } from './lib/server-error-logging';
...
app.get('/api/health/deep', async (c) => {
    ...
    const expectedToken = c.env.DEEP_HEALTH_TOKEN;
    const providedToken = c.req.header('x-health-token');
    if (!isLocalEnv) {
        if (!expectedToken) {
            return c.json({ error: 'deep health disabled', reason: 'DEEP_HEALTH_TOKEN not configured' }, 403);
        }
...
app.get('/api/rx/:token', async (c) => {
  ...
  `).bind(token).first();
    if (!rx)
        return c.json({ error: 'Prescription not found or link expired' }, 404);
    // Check expiry
    const expiresAt = rx.share_expires_at;
    if (expiresAt && new Date(expiresAt) < new Date()) {
        return c.json({ error: 'This share link has expired' }, 410);
    }
    // Fetch items
    const { results: items } = await c.env.DB.prepare('SELECT * FROM prescription_items WHERE prescription_id = ? ORDER BY sort_order').bind(rx.id).all();
    // Fetch hospital name
    const setting = await c.env.DB.prepare(`SELECT value FROM settings WHERE tenant_id = ? AND key = 'hospital_name'`).bind(rx.tenant_id).first();
    return c.json({
        prescription: {
            ...rx,
...
function getPatientPortalCorsOrigin(c) {
...
app.use('/api/*', centralRoutePermissionShadowFromEnv());
...
app.get('/api/tenant/setup-status', async (c) => {
    ...
    try {
        // Count doctors
        const doctorCount = await db.prepare(`SELECT COUNT(*) as count FROM doctors WHERE tenant_id = ? AND is_active = 1`).bind(tenantId).first();
        // Count marketplace-published doctors
        const publishedDoctorCount = await db.prepare(`SELECT COUNT(*) as count FROM doctors WHERE tenant_id = ? AND is_active = 1 AND is_marketplace_visible = 1`).bind(tenantId).first();
        // Check hospital marketplace status
        const hospital = await db.prepare(`SELECT is_published, public_description, specialties, public_photos, latitude, longitude FROM tenants WHERE id = ?`).bind(tenantId).first();
        // Check if any schedules exist
        const scheduleCount = await db.prepare(`SELECT COUNT(*) as count FROM doctor_schedules WHERE tenant_id = ?`).bind(tenantId).first();
        return c.json({
            doctors: {
...
app.onError((err, c) => {
    ...
    if (status >= 400 && status < 500) {
        return c.json({ error: err.message, message: err.message }, status);
    }
