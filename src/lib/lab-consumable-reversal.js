...
async function loadUsageMovements(db, tenantId, labOrderItemId) {
  try {
...
async function loadReversedMovementIds(db, tenantId, labOrderItemId) {
  try {
    const { results } = await db.prepare(`
...
function resolvedStockId(movement, ledgerType) {
  return Number(
    (ledgerType === "inventory" ? movement.inventory_stock_id : movement.lab_stock_id) ?? movement.stock_id ?? 0
  );
}
function reversalGuardStatement(db, tenantId, operationKey, stepKey) {
  return db.prepare(`
...
async function reverseLabConsumableUsageAtomically(db, input) {
  ...
  const statements = [
    db.prepare("DELETE FROM lab_reagent_reversal_guard WHERE tenant_id = ? AND operation_key = ?").bind(String(input.tenantId), operationKey)
  ];
  const legacySnapshots = /* @__PURE__ */ new Map();
  const inventorySnapshots = /* @__PURE__ */ new Map();
  const hasAuditLog = await tableExists(db, "InventoryAuditLog");
  const hasCanonicalConsumption = await tableExists(db, "InventoryConsumption") && await columnExists(db, "InventoryConsumption", "OperationStatus");
  ...
  for (const movement of pendingMovements) {
    ...
    const ledgerType = movement.ledger_type === "inventory" ? "inventory" : "lab";
    ...
    if (ledgerType === "lab") {
      ...
      if (currentQuantityUsed === void 0) {
        const snapshot = await db.prepare(`
          SELECT id, quantity_used
          FROM lab_consumable_stock
          WHERE id = ? AND tenant_id = ?
          LIMIT 1
        `).bind(stockId, input.tenantId).first();
        if (!snapshot) {
      ...
      if (!inventoryStock) {
        inventoryStock = await db.prepare(`
          ...
          LIMIT 1
        `).bind(stockId, input.tenantId).first() ?? void 0;
      }
      if (!inventoryStock) {
...
export {
  reverseLabConsumableUsageAtomically
};
