 ...
 */
export function getRouteActionPermission(prefix, path) {
    const tail = path.slice(prefix.length).replace(/^\/+/, '');
    if (!tail)
        return null;
    // /api/lab/items/:id/verify  →  lab:verify
    if (prefix === '/api/lab/items' && /\/verify\/?$/.test(tail))
        return 'lab:verify';
    // /api/lab/items/:id/publish  →  lab:report:publish
    if (prefix === '/api/lab' && /\/report\/[^/]+\/publish\/?$/.test(tail))
        return 'lab:report:publish';
    // /api/radiology/:id/finalize  →  ris:report:finalize
    if (prefix === '/api/radiology' && /^\/?[^/]+\/finalize\/?$/.test(tail))
        return 'ris:report:finalize';
    // /api/radiology/:id/publish  →  ris:report:publish
    if (prefix === '/api/radiology' && /^\/?[^/]+\/publish\/?$/.test(tail))
        return 'ris:report:publish';
    // /api/radiology/safety/*  →  ris:safety:*
    if (prefix === '/api/radiology' && /^\/?safety(\/|$)/.test(tail)) {
        if (/write|submit|check/.test(tail))
            return 'ris:safety:write';
    ...
    if (prefix.startsWith('/api/hospital-links') || prefix.startsWith('/api/patient-hospital-links') || prefix.startsWith('/api/v1/patients/link-hospital')) {
        if (/^\/?[^/]+\/approve\/?$/.test(tail))
            return 'hospital_link:approve';
        if (/^\/?[^/]+\/consent\/?$/.test(tail))
            return 'hospital_link:consent:write';
    }
 ...
 */
export function getRequiredRoutePermission(path, method) {
    ...
    for (const rule of ROUTE_PERMISSION_MATRIX) {
        if (!isPathUnder(path, rule.prefix))
            continue;
        // Sub-path elevation
 ...
 */
async function resolveEffective(ctx) {
    if (!ctx.role)
        return [];
 ...
 */
export function centralRoutePermission(options = {}) {
    const mode = options.mode ?? 'enforce';
    const report = async (event) => {
        if (options.onViolation) {
            await options.onViolation(event);
        }
    };
    return async (c, next) => {
        ...
        if (!lookup) {
            const violation = { type: 'missing_rule', ...baseEvent };
        ...
        if (Array.isArray(lookup.permission) && lookup.permission.length === 0) {
            const violation = { type: 'method_not_allowed', ...baseEvent, prefix: lookup.prefix };
            await report(violation);
            if (mode === 'shadow') {
                await next();
                return;
        ...
        if (!role) {
            const violation = { type: 'missing_role', ...baseEvent, prefix: lookup.prefix, required };
            await report(violation);
            if (mode === 'shadow') {
                await next();
                return;
        ...
        if (!hasAll) {
            const violation = { type: 'missing_permission', ...baseEvent, prefix: lookup.prefix, required, effectivePermissions: permissions };
            await report(violation);
            if (mode === 'shadow') {
                await next();
                return;
...
export function centralRoutePermissionFromEnv() {
