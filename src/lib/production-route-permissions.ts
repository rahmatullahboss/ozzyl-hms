...
export type ProductionRoutePermissionContext = {
  db: D1Database;
  kv?: KVNamespace;
  tenantId?: string;
  userId?: string;
...
const CORE_ROUTE_PERMISSION_RULES: PermissionRule[] = [
  {
    prefix: '/api/patients',
...
export function getRequiredProductionPermission(path: string, method: string): string | null {
  const normalizedMethod = method.toUpperCase();
  const rule = CORE_ROUTE_PERMISSION_RULES.find((item) => isPathUnder(path, item.prefix));
  return rule?.permission(normalizedMethod, path) ?? null;
}

async function resolveEffectivePermissions(ctx: ProductionRoutePermissionContext): Promise<string[]> {
  if (!ctx.role) return [];
...
export async function enforceProductionRoutePermission(ctx: ProductionRoutePermissionContext): Promise<void> {
  const requiredPermission = getRequiredProductionPermission(ctx.path, ctx.method);
