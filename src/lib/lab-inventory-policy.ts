export type LabReagentConsumptionTiming = 'billing' | 'result';
export type LabReagentConsumptionEvent = 'billing' | 'result';
export type LabInventoryMode = 'disabled' | 'soft' | 'strict';

export interface LabInventoryPolicy {
  lab_inventory_mode: LabInventoryMode;
  reagent_consumption_timing: LabReagentConsumptionTiming;
  allow_result_without_stock: boolean;
  require_test_mapping_for_completion: boolean;
}

export const LAB_INVENTORY_CAPABILITIES = {
...
export interface LabInventoryStrictModeReadiness {
  ready: boolean;
  score: number;
  counts: {
    ...
    activeBillingTests: number;
    mappedBillingTests: number;
    missingBillingTests: number;
    coveragePercent: number;
    openExceptions: number;
    openStockShortageExceptions: number;
    qcRiskLots: number;
    onboardExpiryRiskLots: number;
  };
  blockers: string[];
  warnings: string[];
  nextActions: Array<{ id: string; title: string; action: string }>;
}

export const DEFAULT_LAB_INVENTORY_POLICY: LabInventoryPolicy = {
...
export function normalizeLabReagentConsumptionTiming(value: unknown): LabReagentConsumptionTiming {
  return value === 'result' ? 'result' : 'billing';
}

export function normalizeLabInventoryMode(value: unknown): LabInventoryMode {
...
export function normalizeLabInventoryPolicy(row?: Partial<Record<keyof LabInventoryPolicy, unknown>> | null): LabInventoryPolicy {
  return {
...
export function shouldBlockLabInventoryException(policy: LabInventoryPolicy, event: LabReagentConsumptionEvent): boolean {
  if (policy.lab_inventory_mode !== 'strict') return false;
  if (event === 'result' && policy.allow_result_without_stock) return false;
  return true;
}

export async function getLabInventoryPolicy(db: D1Database, tenantId: string | number): Promise<LabInventoryPolicy> {
  try {
    ...
    `).bind(String(tenantId)).first<{
      lab_inventory_mode?: string | null;
      reagent_consumption_timing?: string | null;
      allow_result_without_stock?: number | boolean | string | null;
      require_test_mapping_for_completion?: number | boolean | string | null;
    }>();
...
export async function shouldConsumeLabReagentsForEvent(
  db: D1Database,
  tenantId: string | number,
  event: LabReagentConsumptionEvent,
): Promise<boolean> {
...
type D1BindValue = string | number | boolean | null;
...
async function countRows(db: D1Database, sql: string, ...params: unknown[]): Promise<number> {
  const row = await db.prepare(sql).bind(...params.map(normalizeBindValue)).first<{ count?: number | string | null }>();
  const parsed = Number(row?.count ?? 0);
  return Number.isFinite(parsed) ? parsed : 0;
}

async function safeCount(db: D1Database, sql: string, ...params: unknown[]): Promise<number> {
  try {
...
export async function getLabInventoryStrictModeReadiness(
  db: D1Database,
  tenantId: string | number,
  referenceDate = new Date(),
): Promise<LabInventoryStrictModeReadiness> {
  ...
  const coveragePercent = activeBillingTests > 0 ? Math.round((mappedBillingTests / activeBillingTests) * 10000) / 100 : 0;

  const blockers: string[] = [];
  const warnings: string[] = [];
  const nextActions: Array<{ id: string; title: string; action: string }> = [];
...
export async function upsertLabInventoryPolicy(
  db: D1Database,
  input: {
    tenantId: string | number;
