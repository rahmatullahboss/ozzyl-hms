...
import { listLabReagentReconciliation, type LabReagentReconciliationStatus } from '../../lib/lab-reagent-reconciliation';
import { backfillLegacyReagentStock, createCanonicalReagentStock, isCanonicalReagentCategory } from '../../lib/lab-reagent-stock-sync';
...
const labMonitoring = new Hono<{ Bindings: Env; Variables: Variables }>();

labMonitoring.use('*', requireRole('laboratory', 'lab', 'lab_tech', 'hospital_admin', 'director', 'receptionist'));

const requireLabInventoryRole = (...allowedRoles: string[]) => async (c: any, next: any) => {
...
type GoLiveCheckStatus = 'ready' | 'warning' | 'blocked';

function goLiveCheck(id: string, label: string, status: GoLiveCheckStatus, detail: string, action?: string) {
  return { id, label, status, detail, action: action ?? null };
}

function goLiveOverall(checks: Array<{ status: GoLiveCheckStatus }>): GoLiveCheckStatus {
  if (checks.some((check) => check.status === 'blocked')) return 'blocked';
  if (checks.some((check) => check.status === 'warning')) return 'warning';
  return 'ready';
}

function goLiveScore(checks: Array<{ status: GoLiveCheckStatus }>): number {
...
labMonitoring.get('/inventory-reconciliation', labInventoryManagerOnly, async (c) => {
  ...
  const allowedStatuses = new Set(['all', 'complete', 'partial', 'projection_missing', 'mismatch']);
  const status = allowedStatuses.has(requestedStatus)
    ? requestedStatus as LabReagentReconciliationStatus | 'all'
    : 'all';
...
labMonitoring.put('/inventory-policy', labInventoryAdminOnly, zValidator('json', labInventoryPolicySchema), async (c) => {
  ...
  let strictModeReadiness = null as Awaited<ReturnType<typeof getLabInventoryStrictModeReadiness>> | null;
...
labMonitoring.post('/inventory-exceptions/:id/retry-consumption', labInventoryManagerOnly, async (c) => {
  ...
  try {
    ...
    if (Number((updateResult.meta as { changes?: number } | undefined)?.changes ?? 0) !== 1) {
...
labMonitoring.get('/consumables', async (c) => {
  ...
  if (search) { where += ' AND (c.name LIKE ? OR c.code LIKE ?)'; params.push(`%${search}%`, `%${search}%`); }
  ...
  let results = consumables.results as Array<Record<string, unknown>>;
...
labMonitoring.get('/consumables/:id', async (c) => {
  ...
  const linkedInventoryItemId = Number((consumable as Record<string, unknown>).inventory_item_id ?? 0);
  if (linkedInventoryItemId > 0) {
    try {
      ...
      `).bind(linkedInventoryItemId, tenantId).first<{ total_stock: number }>();
      (consumable as Record<string, unknown>).total_stock = Number(inventoryTotal?.total_stock ?? 0);
    } catch {
      try {
        const inventoryTotal = await db.$client.prepare(`
          SELECT COALESCE(SUM(AvailableQuantity), 0) as total_stock
        ...
        // Keep legacy projection when canonical inventory tables are unavailable.
      }
    }
  }
...
labMonitoring.post('/consumables', labInventoryManagerOnly, zValidator('json', consumableSchema), async (c) => {
  ...
  `).bind(
    ...
    data.unit_price ?? 0, data.reorder_level ?? 10, data.reorder_qty ?? 50,
    data.supplier_id ?? null, data.description ?? null, data.storage_condition ?? null,
    data.expiry_alert_days ?? 30, tenantId, userId,
  ).run();
...
labMonitoring.put('/consumables/:id', labInventoryManagerOnly, zValidator('json', consumableSchema.partial()), async (c) => {
  ...
  if (data.reorder_level !== undefined) { sets.push('reorder_level = ?'); vals.push(data.reorder_level); }
  if (data.reorder_qty !== undefined) { sets.push('reorder_qty = ?'); vals.push(data.reorder_qty); }
  if (data.supplier_id !== undefined) { sets.push('supplier_id = ?'); vals.push(data.supplier_id); }
  if (data.description !== undefined) { sets.push('description = ?'); vals.push(data.description); }
  if (data.storage_condition !== undefined) { sets.push('storage_condition = ?'); vals.push(data.storage_condition); }
  if (data.expiry_alert_days !== undefined) { sets.push('expiry_alert_days = ?'); vals.push(data.expiry_alert_days); }
...
labMonitoring.get('/analyzer-health', async (c) => {
  ...
  try {
    ...
    try {
      const row = await db.$client.prepare('SELECT COUNT(*) as count FROM lab_machines').first<{ count: number }>();
      data.machines_total = Number(row?.count ?? 0);
    } catch {}
  }
  ...
  try {
    ...
    `).bind(tenantId).first<{ active_assignments: number; machines_with_active_assignment: number }>();
  ...
  try {
    const row = await db.$client.prepare(`
      ...
      WHERE inv.tenant_id = ?
        ...
        AND (inv.AfterOpenExpiryDate IS NULL OR date(inv.AfterOpenExpiryDate) >= CURRENT_DATE)
        AND (inv.ExpiryDate IS NULL OR inv.ExpiryDate = '' OR date(inv.ExpiryDate) >= CURRENT_DATE)
    `).bind(tenantId).first<{ inventory_reagent_lots: number; unassigned_inventory_lots: number }>();
...
labMonitoring.get('/lis-go-live-readiness', async (c) => {
  ...
  const checks: Array<ReturnType<typeof goLiveCheck>> = [];
  ...
  ).bind(...machineBinds).first<{ count: number }>();
  ...
  const bridgeAgents = await db.$client.prepare(`
    ...
    WHERE tenant_id = ?
  `).bind(tenantId).first<{ total: number; healthy: number }>();
  ...
  `).bind(tenantId, tenantId).first<{ controls: number; ranges: number }>();
  ...
  `).bind(...machineBinds).first<{ total: number; needs_review: number }>();
...
labMonitoring.get('/lis-bridge-deployment-checklist', async (c) => {
  ...
  let machine: MachineChecklistRow | null = null;
  ...
  if (machineId) {
    machine = await db.$client.prepare(`
      ...
      LIMIT 1
    `).bind(tenantId, machineId).first<MachineChecklistRow>();
...
const manualUsageTypeValues = ['manual', 'rerun', 'control', 'qc', 'calibration', 'other'] as const;

function manualUsageReferenceType(usageType: typeof manualUsageTypeValues[number]): string {
...
labMonitoring.post('/consumables/:id/manual-usage', labInventoryOperatorOnly, zValidator('json', manualUsageSchema), async (c) => {
  ...
  ).bind(consumableId, tenantId).first<{ id: number; name?: string | null }>();
  if (!consumable) throw new HTTPException(404, { message: 'Consumable not found' });

  const referenceType = manualUsageReferenceType(data.usage_type);
  const remarks = data.remarks;
  const result = await consumeLabConsumableStock(db.$client as unknown as D1Database, {
    tenantId,
    userId,
    consumableId,
...
labMonitoring.get('/stock/lots', async (c) => {
  ...
  const queryParams: Array<string | number> = consumableId ? [tenantId, consumableId] : [tenantId];
  ...
  try {
    const legacy = await db.$client.prepare(`
      ...
      WHERE s.tenant_id = ?
        ...
        AND (s.expiry_date IS NULL OR date(s.expiry_date) >= CURRENT_DATE)
        ${consumableFilter}
    `).bind(...queryParams).all<Record<string, unknown>>();
...
labMonitoring.post('/stock/in', labInventoryOperatorOnly, zValidator('json', stockInSchema), async (c) => {
  ...
  const consumable = await db.$client.prepare(`
    ...
    LIMIT 1
  `).bind(data.consumable_id, tenantId).first<{ category: string }>();
  if (!consumable) throw new HTTPException(404, { message: 'Consumable not found' });

  if (!isCanonicalReagentCategory(consumable.category)) {
    const qcStatus = 'not_required' as const;
    const stockResult = await db.$client.prepare(`
      INSERT INTO lab_consumable_stock
        (consumable_id, lot_number, expiry_date, quantity_received, purchase_price,
         received_date, remarks, qc_status, location_id, tenant_id, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      data.consumable_id,
    ...
    await db.$client.prepare(`
      INSERT INTO lab_consumable_movements
        (consumable_id, stock_id, movement_type, quantity, unit_cost,
         reference_type, performed_by, remarks, tenant_id)
    ...
    return c.json({ id: stockResult.meta.last_row_id, message: 'Stock added', qc_status: qcStatus }, 201);
  }

  try {
    const result = await createCanonicalReagentStock(c.env.DB, {
      tenantId,
      userId,
    ...
    const body = {
      id: result.labStockId,
      inventory_stock_id: result.inventoryStockId,
      inventory_item_id: result.inventoryItemId,
      inventory_store_id: result.inventoryStoreId,
      message: result.deduplicated ? 'Stock already added' : 'Stock added',
      qc_status: result.qcStatus,
      deduplicated: result.deduplicated,
    };
    return c.json(body, result.deduplicated ? 200 : 201);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to add reagent stock';
    const status = message === 'Consumable not found' || message === 'Lab stock location not found' ? 404 : 400;
    throw new HTTPException(status, { message });
  }
});

labMonitoring.post('/stock/backfill-canonical', labInventoryManagerOnly, async (c) => {
  const summary = await backfillLegacyReagentStock(c.env.DB, {
    tenantId: requireTenantId(c),
    userId: requireUserId(c),
  });
  return c.json({ message: 'Legacy reagent stock sync completed', summary });
});

type LinkedInventoryLabStock = {
  stock_id: number;
  item_id: number;
  ...
  after_open_expiry_date?: string | null;
  active_assignment_id?: number | null;
  assigned_machine_id?: number | null;
  assigned_location_id?: number | null;
};

async function loadLinkedInventoryLabStock(
  db: ReturnType<typeof getDb>,
  tenantId: string,
  stockId: number,
): Promise<LinkedInventoryLabStock | null> {
  try {
    return await db.$client.prepare(`
      ...
      LIMIT 1
    `).bind(stockId, tenantId).first<LinkedInventoryLabStock>();
  } catch {
    try {
      return await db.$client.prepare(`
        SELECT
...
async function loadLabMachineForAssignment(
  db: ReturnType<typeof getDb>,
  tenantId: string,
  machineId: number,
): Promise<{ id: number; machine_name?: string | null; machine_code?: string | null } | null> {
  try {
    return await db.$client.prepare(`
      ...
      WHERE id = ? AND tenant_id = ? AND COALESCE(is_active, 1) = 1
      LIMIT 1
    `).bind(machineId, tenantId).first<{ id: number; machine_name?: string | null; machine_code?: string | null }>();
  } catch {
    try {
      return await db.$client.prepare(`
        SELECT id
        ...
        WHERE id = ?
        LIMIT 1
      `).bind(machineId).first<{ id: number; machine_name?: string | null; machine_code?: string | null }>();
    } catch {
      return null;
    }
  }
...
async function loadLabStockLocationForAssignment(
  db: ReturnType<typeof getDb>,
  tenantId: string,
  locationId: number,
): Promise<{ id: number; location_name?: string | null; location_type?: string | null } | null> {
  try {
    return await db.$client.prepare(`
      ...
      WHERE id = ? AND tenant_id = ? AND is_active = 1
      LIMIT 1
    `).bind(locationId, tenantId).first<{ id: number; location_name?: string | null; location_type?: string | null }>();
  } catch {
    return null;
  }
}

function inventoryStockStatusForQc(qcStatus: string): string {
  return qcStatus === 'failed' || qcStatus === 'pending' ? 'blocked' : 'available';
}

async function logInventoryMetadataAudit(
  db: ReturnType<typeof getDb>,
  input: {
    tenantId: string;
    userId: string;
    stock: LinkedInventoryLabStock;
    action: string;
  ...
  try {
    ...
    // Older partial inventory schemas may not have InventoryAuditLog yet.
  }
}
...
labMonitoring.post('/stock/:stockId/qc', labInventoryOperatorOnly, zValidator('json', stockQcSchema), async (c) => {
  ...
  `).bind(stockId, tenantId).first<{ id: number; consumable_id: number }>();
...
labMonitoring.post('/stock/:stockId/open', labInventoryOperatorOnly, zValidator('json', stockOpenSchema), async (c) => {
  ...
  if (stock) {
    ...
    const openedLot = await db.$client.prepare(`
      ...
      FROM lab_consumable_stock
      WHERE id = ? AND tenant_id = ?
    `).bind(stockId, tenantId).first<{ opened_at?: string | null; onboard_expires_at?: string | null }>();
  ...
  const openedInventoryLot = await db.$client.prepare(`
    ...
    FROM InventoryStock
    WHERE StockId = ? AND tenant_id = ?
  `).bind(stockId, tenantId).first<{ OpenDate?: string | null; AfterOpenExpiryDate?: string | null }>();
...
labMonitoring.post('/stock/:stockId/transfer-location', labInventoryOperatorOnly, zValidator('json', stockTransferSchema), async (c) => {
  ...
  const targetLocation = await db.$client.prepare(`
    ...
    FROM lab_consumable_locations
    WHERE id = ? AND tenant_id = ? AND is_active = 1
  `).bind(data.target_location_id, tenantId).first<{ id: number; location_name: string }>();
...
const wasteRemarksSchema = z.string().trim().max(500).optional().transform(value => value || undefined);
...
labMonitoring.post('/stock/waste-requests', labInventoryOperatorOnly, zValidator('json', wasteRequestSchema), async (c) => {
  ...
  `).bind(data.stock_id, tenantId).first<{ id: number; consumable_id: number; quantity_available: number }>();
...
}).refine((data) => data.qty_per_test !== undefined || data.is_mandatory !== undefined || data.notes !== undefined, {
  message: 'Provide at least one mapping field to update',
});

async function assertTestConsumableMapReferences(
  dbClient: D1Database,
  tenantId: string | number,
  mapping: { lab_test_id: number; consumable_id: number },
): Promise<void> {
  const labTest = await dbClient.prepare(`
    ...
    WHERE id = ? AND tenant_id = ? AND COALESCE(is_active, 1) = 1
    LIMIT 1
  `).bind(mapping.lab_test_id, tenantId).first<{ id: number }>();
  if (!labTest?.id) {
  ...
  const consumable = await dbClient.prepare(`
    ...
    WHERE id = ? AND tenant_id = ? AND COALESCE(is_active, 1) = 1
    LIMIT 1
  `).bind(mapping.consumable_id, tenantId).first<{ id: number }>();
  if (!consumable?.id) {
...
labMonitoring.get('/mapping-coverage', async (c) => {
  ...
  let mappingCoverageResult: { results?: any[] };
  try {
    mappingCoverageResult = await c.env.DB.prepare(`
      ...
      LIMIT ?
    `).bind(String(tenantId), includeOutsourced ? 1 : 0, limit).all<any>();
  } catch {
    mappingCoverageResult = await c.env.DB.prepare(`
      SELECT
        t.id,
  ...
  const coveragePercent = summary.total_tests > 0 ? Math.round((summary.mapped_tests / summary.total_tests) * 10000) / 100 : 0;

  let qcFailedLotRow: { qc_failed_usable_lot_count: number } | null;
  try {
    ...
    `).bind(String(tenantId), String(tenantId)).first<{ qc_failed_usable_lot_count: number }>();
  } catch {
    qcFailedLotRow = await c.env.DB.prepare(`
      ...
      WHERE s.tenant_id = ?
        ...
        AND LOWER(COALESCE(s.qc_status, 'not_required')) = 'failed'
        AND (s.expiry_date IS NULL OR date(s.expiry_date) >= CURRENT_DATE)
    `).bind(String(tenantId)).first<{ qc_failed_usable_lot_count: number }>();
  }
  ...
  `).bind(String(tenantId)).first<{ open_stock_shortage_exceptions: number }>();
  ...
  const strictModeReady = summary.total_tests > 0
    && coveragePercent >= coverageTargetMin
    && summary.missing_tests === 0
    && qcFailedUsableLots === 0
    && openStockShortageExceptions === 0;
...
labMonitoring.get('/test-consumable-map', async (c) => {
  ...
  const params: (string | number)[] = [tenantId];
  if (testId) { where += ' AND m.lab_test_id = ?'; params.push(Number(testId)); }
...
labMonitoring.post('/test-consumable-map/bulk', labInventoryManagerOnly, zValidator('json', bulkTestConsumableMapSchema), async (c) => {
  ...
  let reactivated = 0;
  let skipped = 0;
  const details: Array<{ lab_test_id: number; consumable_id: number; status: string; id?: number }> = [];
  ...
  for (const mapping of data.mappings) {
    ...
    const existing = await db.$client.prepare(`
      ...
      LIMIT 1
    `).bind(mapping.lab_test_id, mapping.consumable_id, tenantId).first<{ id: number; is_active: number }>();
...
labMonitoring.post('/test-consumable-map', labInventoryManagerOnly, zValidator('json', testConsumableMapSchema), async (c) => {
  ...
  const existing = await db.$client.prepare(`
    ...
    WHERE lab_test_id = ? AND consumable_id = ? AND tenant_id = ?
    LIMIT 1
  `).bind(data.lab_test_id, data.consumable_id, tenantId).first<{ id: number; is_active: number }>();
...
labMonitoring.put('/test-consumable-map/:id', labInventoryManagerOnly, zValidator('json', testConsumableMapUpdateSchema), async (c) => {
  ...
  `).bind(
    ...
    data.is_mandatory === undefined ? null : (data.is_mandatory ? 1 : 0),
...
labMonitoring.get('/operation-logs', async (c) => {
  ...
  if (logType) { where += ' AND l.log_type = ?'; params.push(logType); }
...
labMonitoring.post('/operation-logs', labInventoryOperatorOnly, zValidator('json', operationLogSchema), async (c) => {
  ...
  const result = await db.$client.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    data.log_type, data.lab_test_id ?? null, data.consumable_id ?? null,
    data.lab_order_id ?? null, data.radiology_req_id ?? null, data.quantity,
    data.machine_id ?? null, data.description ?? null, userId, tenantId,
  ).run();
...
// ═══════════════════════════════════════════════════════════════════════════════
// 5. DAILY SUMMARY (computed on-demand or pre-computed)
// ═══════════════════════════════════════════════════════════════════════════════
...
labMonitoring.get('/daily-summary', async (c) => {
  ...
  `).bind(tenantId, date).first<{ total: number; done: number; pending: number }>();
  ...
  `).bind(tenantId, date).first<{ total: number | null }>();
  ...
  let reagents: { total: number | null } | null = null;
  try {
...
labMonitoring.get('/reagent-reconciliation', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    LIMIT ?
  `).bind(String(tenantId), from, to, limit).all<any>();
  ...
  const rows = (results ?? []).map((row: any) => {
    ...
    const tatMinutes = row.tat_minutes_actual === null || row.tat_minutes_actual === undefined ? null : Number(row.tat_minutes_actual);
    const tatStatus = tatMinutes === null
      ? 'pending'
      : tatTargetMinutes && tatMinutes > tatTargetMinutes
        ? 'delayed'
        : 'on_time';
    const rowStatus = exceptionCount > 0
      ? 'exception'
      : expectedQty <= 0 || consumedQty < expectedQty
        ? 'missing'
        : 'ok';
    const statusMeaning = rowStatus === 'ok'
      ? 'Expected reagent deducted'
      : rowStatus === 'exception'
        ? 'Deduction failed/needs review'
        : 'Mapping/stock missing';
    return {
...
labMonitoring.post('/film-usage', labInventoryOperatorOnly, zValidator('json', filmUsageSchema), async (c) => {
  ...
  const result = await db.$client.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    data.requisition_id, data.film_type_id, data.film_size ?? null,
    data.quantity_used, data.quantity_wasted, data.print_count,
    userId, data.remarks ?? null, tenantId,
  ).run();

  // Also log as operation log
  await db.$client.prepare(`
    INSERT INTO lab_operation_logs
  ...
  // Mark requisition as film usage logged
  await db.$client.prepare(
...
labMonitoring.post('/template-presets', labInventoryManagerOnly, zValidator('json', presetSchema), async (c) => {
  ...
  const result = await db.$client.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)
  `).bind(
    data.preset_code, data.preset_name, data.preset_name_bn ?? null,
    data.category, data.layout_type, data.structure_json, data.sample_html ?? null, tenantId,
  ).run();
...
labMonitoring.get('/help', async (c) => {
  ...
  const lang = c.req.query('lang') || 'bn'; // default Bengali for Bangladesh hospitals
...
// ═══════════════════════════════════════════════════════════════════════════════
// 10. CRITICAL LAB ALERTS (legacy endpoint for LaboratoryDashboard)
// ═══════════════════════════════════════════════════════════════════════════════
...
labMonitoring.post('/critical/:alertId/acknowledge', labInventoryOperatorOnly, async (c) => {
  ...
  `).bind(alertId, tenantId).first<Record<string, unknown>>();
