...
import { createBloodSugarSchema, updateBloodSugarSchema, bloodSugarQuerySchema } from '../../../schemas/nursing';
import { getDb } from '../../../db';

type NursingEnv = { Bindings: Env; Variables: Variables };
const ALLOWED_UPDATE_FIELDS = ['rbs_value', 'insulin', 'remarks', 'entry_datetime'];
...
bloodSugarRoutes.get('/', zValidator('query', bloodSugarQuerySchema), async (c) => {
  ...
  let query = `
    SELECT id, tenant_id, patient_id, visit_id, entry_datetime, rbs_value, insulin, remarks, created_by, created_at
    FROM nur_blood_sugar_monitoring WHERE tenant_id = ? AND is_active = 1
  `;
...
bloodSugarRoutes.get('/:id', async (c) => {
  ...
  const result = await db.$client.prepare(`
    SELECT id, tenant_id, patient_id, visit_id, entry_datetime, rbs_value, insulin, remarks, created_by, created_at
    FROM nur_blood_sugar_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1
...
bloodSugarRoutes.post('/', zValidator('json', createBloodSugarSchema), async (c) => {
  ...
  `).bind(
    ...
    data.entry_datetime ?? null, userId
  ).run();
...
bloodSugarRoutes.put('/:id', zValidator('json', updateBloodSugarSchema), async (c) => {
  ...
  const id = parseInt(c.req.param('id'));
  if (isNaN(id)) throw new HTTPException(400, { message: 'Invalid ID' });
  const existing = await db.$client.prepare('SELECT 1 FROM nur_blood_sugar_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  ...
  if (fields.length > 0) {
    ...
    await db.$client.prepare(`UPDATE nur_blood_sugar_monitoring SET ${fields.join(', ')} WHERE id = ? AND tenant_id = ?`).bind(...values).run();
  }
...
bloodSugarRoutes.delete('/:id', async (c) => {
  ...
  const existing = await db.$client.prepare('SELECT 1 FROM nur_blood_sugar_monitoring WHERE id = ? AND tenant_id = ? AND is_active = 1').bind(id, tenantId).first();
  if (!existing) throw new HTTPException(404, { message: 'Not found' });
  await db.$client.prepare("UPDATE nur_blood_sugar_monitoring SET is_active = 0, updated_at = datetime('now', '+6 hours') WHERE id = ? AND tenant_id = ?").bind(id, tenantId).run();
  return c.json({ Results: true });
});
