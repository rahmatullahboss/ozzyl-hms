...
const REPORT_ROLES = ["hospital_admin", "md", "director", "accountant", "reception", "receptionist"];
const SUPERVISOR_ROLES = /* @__PURE__ */ new Set(["hospital_admin", "md", "director", "accountant"]);
const n = (value) => {
...
const denominationNotes = [1e3, 500, 200, 100, 50, 20, 10, 5, 2, 1];
function parseDenominationSnapshot(raw) {
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    return denominationNotes.map((note) => {
      const countValue = Number(parsed[`note${note}`] ?? 0);
      const count2 = Number.isFinite(countValue) ? Math.max(0, Math.round(countValue)) : 0;
      return { note, count: count2, total: count2 * note };
    }).filter((row) => row.count > 0);
  } catch {
    return [];
  }
}
function parseSettlementSnapshot(raw) {
  if (!raw) return {};
  try {
    const parsed = JSON.parse(raw);
    const normalized = {};
...
async function loadSession(c, tenantId, userId, sessionId) {
  if (sessionId) {
...
async function loadFinalSnapshot(c, tenantId, sessionId) {
  return c.env.DB.prepare(`
...
function snapshotListItem(snapshot) {
  const report = parseSnapshot(snapshot);
  return {
    id: Number(snapshot.id),
...
function ensureCanAccessReport(role, userId, cashierId) {
  if (!SUPERVISOR_ROLES.has(role) && Number(cashierId) !== userId) {
    throw new HTTPException(403, { message: "Receptionists can only view their own shift handover report" });
  }
}
async function buildLiveReport(c, tenantId, userId, role, session) {
  ...
  const reportDate = (startAt || (/* @__PURE__ */ new Date()).toISOString()).slice(0, 10);
  ...
  `).bind(tenantId, session.employee_id, startAt, endAt, endAt).first();
  ...
  const { results: transferRows } = await c.env.DB.prepare(`
    ...
    LIMIT 50
  `).bind(tenantId, session.id).all();
  const handoverRow = await c.env.DB.prepare(`
    SELECT h.handover_amount, h.due_amount, h.received_amount, h.status, h.remarks,
  ...
  const { results: approvalRequestRows } = await c.env.DB.prepare(`
    ...
    LIMIT 50
  `).bind(tenantId, session.id, session.employee_id, startAt, endAt, endAt).all();
  ...
  return {
    ...
    audit: {
      ...
      generatedBy: userId,
      scope: SUPERVISOR_ROLES.has(role) ? "supervisor" : "own_shift"
    }
  };
}
...
routes.get("/history", async (c) => {
  ...
  const filters = ["r.tenant_id = ?", "r.status IN ('finalized', 'accepted')"];
...
routes.post("/sessions/:sessionId/accept", async (c) => {
  ...
  const sessionId = parsePositiveInt(c.req.param("sessionId"), "sessionId");
  if (!sessionId) throw new HTTPException(400, { message: "sessionId is required" });
  if (!SUPERVISOR_ROLES.has(role)) throw new HTTPException(403, { message: "Only supervisors can accept shift handover reports" });
  const existing = await loadFinalSnapshot(c, tenantId, sessionId);
...
var shiftHandoverReport_default = routes;
export {
  shiftHandoverReport_default as default
};
