...
Status: Done. Manager dashboard now uses dedicated web/src/pages/ManagerDashboard.tsx, App lazy route wiring, default manager redirect, API-backed KPI cards, operations board, alerts/follow-up panel, quick links, and page contract tests.
...
Status: Done in src/routes/tenant/managerDashboard.ts. Added API summary route, tenant-scoped role plus manager.dashboard.read permission guard, date validation, safe summary shape, basic alerts, and route contract tests.
...
Status: Done. Header workspace switcher now hard-bounds manager users to Manager, Reception, and Lab before MD, Director, or Accountant options are considered. Added source-level tests in test/manager-dashboard-switcher.test.ts.
...
Status: Done. Added and ran manager dashboard authz, summary route, workspace switcher, and page wiring contract tests. Review follow-up: manager default route now points to manager/dashboard, manager reports module visibility is false by default, and Header no longer carries manager in the MD workspace condition.
