...
function OrderStatusBadge({ status }: { status: string }) {
  ...
  const statusLabelMap: Record<string, string> = {
    ...
    on_hold: 'medicationOrders.status.onHold',
...
function NewOrderModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(axios.isAxiosError(err) ? err.response?.data?.message ?? t('medicationOrders.failed') : t('medicationOrders.failed'));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="label">{t('medicationOrders.discontinue.reason')} (optional)</label>
          ...
          <table className="table-base">
            <thead>
              <tr>
                ...
                <th>{t('medicationOrders.status')}</th>
