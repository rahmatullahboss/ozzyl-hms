...
export default function HospitalProfile() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setConnectMsg(res.ok ? (data.already_connected ? 'Already connected' : 'Connected!') : (data.error || 'Failed'));
    } catch {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>/</span>
          <Link to="/marketplace/hospitals" className="hover:text-blue-600">Hospitals</Link>
          <span>/</span>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {connectMsg && <p className="text-sm text-green-600 mt-1">{connectMsg}</p>}
          ...
          {specialties.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-4">
              {specialties.map((s) => (
                <span key={s} className="text-sm px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full capitalize">
                  {s}
                </span>
              ))}
            </div>
        ...
        {Object.keys(operatingHours).length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <h2 className="font-semibold text-gray-900 mb-3">Operating Hours</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {Object.entries(operatingHours).map(([day, hours]) => (
                <div key={day} className="text-sm">
                  <span className="font-medium capitalize text-gray-700">{day}: </span>
                  <span className={hours === 'closed' ? 'text-red-400' : 'text-gray-600'}>{hours}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {doctors.length > 0 && (
          <div>
            <h2 className="font-semibold text-gray-900 mb-3">Doctors</h2>
