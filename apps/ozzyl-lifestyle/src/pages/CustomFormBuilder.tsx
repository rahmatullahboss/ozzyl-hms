...
export default function CustomFormBuilder({ role }: { role?: string }) {
  ...
  const [forms, setForms] = useState<LbfForm[]>([]);
  ...
  const loadForms = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/lbf-forms', { headers: authHeaders() });
      setForms(res.data?.Results || []);
    } catch {
      toast.error(t('failedToLoadForms'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadFormFields = async (formId: number) => {
    try {
      const res = await axios.get(`/api/lbf-forms/${formId}`, { headers: authHeaders() });
      setFormFields(res.data?.Results?.fields || []);
    } catch {
  ...
  const handleCreateForm = async (e: React.FormEvent) => {
    ...
    setSavingForm(true);
    try {
      await axios.post('/api/lbf-forms', {
        FormName: newFormName.toLowerCase().replace(/[^a-z0-9_]/g, '_'), // Ensure valid format
      ...
      toast.error(err.response?.data?.error || t('failedToCreateForm'));
    } finally {
      setSavingForm(false);
    }
  };
  ...
  const handleCreateField = async (e: React.FormEvent) => {
    ...
    setSavingField(true);
    try {
      await axios.post(`/api/lbf-forms/${selectedForm.FormId}/fields`, {
      ...
      setNewFieldName('');
      setNewFieldLabel('');
      loadFormFields(selectedForm.FormId);
    } catch (err: any) {
      toast.error(err.response?.data?.error || t('failedToAddField'));
    } finally {
          ...
          <div className="bg-[var(--color-bg-primary)] rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleCreateForm} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-2">
                ...
                <button type="submit" disabled={savingForm} className="btn btn-primary min-w-[100px]">{savingForm ? t('saving') : t('createFormBtn')}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-[var(--color-bg-primary)] rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleCreateField} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-2">
                ...
                <button type="submit" disabled={savingField} className="btn bg-purple-600 text-white hover:bg-purple-700 min-w-[100px]">{savingField ? t('saving') : t('addFieldBtn')}</button>
              </div>
            </form>
          </div>
        </div>
