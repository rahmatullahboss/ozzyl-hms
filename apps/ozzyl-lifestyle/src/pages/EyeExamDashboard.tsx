...
export default function EyeExamDashboard({ role }: { role?: string }) {
  ...
  const loadExams = async (patientId: number) => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/clinical/eye-exam?patientId=${patientId}`, { headers: authHeaders() });
      setExams(res.data?.Results ?? []);
    } catch {
      toast.error(t('clinical.failed_to_load_eye_exams'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveExam = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create base exam
      const resBase = await axios.post('/api/clinical/eye-exam', {
      ...
      if (form.MRODSPH || form.MROSSPH || form.MRODCYL || form.MROSCYL) {
        await axios.post('/api/clinical/eye-exam/refraction', {
          EyeExamId: eyeExamId, PatientId: selectedPatient.id,
      ...
      if (form.ODCORNEA || form.OSCORNEA || form.ODLENS || form.OSLENS || form.ODIOPNCT || form.OSIOPNCT) {
         await axios.post('/api/clinical/eye-exam/anterior-segment', {
          EyeExamId: eyeExamId, PatientId: selectedPatient.id,
      ...
      if (form.ODMACULA || form.OSMACULA || form.ODDISC || form.OSDISC) {
        await axios.post('/api/clinical/eye-exam/fundus', {
          EyeExamId: eyeExamId, PatientId: selectedPatient.id,
      ...
      if (form.DiagnosisOD || form.DiagnosisOS || form.Assessment || form.Plan) {
        await axios.post('/api/clinical/eye-exam/assessment', {
          EyeExamId: eyeExamId, PatientId: selectedPatient.id,
      ...
      toast.success(t('clinical.eye_exam_saved_successfully'));
      setShowModal(false);
      loadExams(selectedPatient.id);
    } catch (err) {
      toast.error(t('clinical.failed_to_save_comprehensive_eye_exam'));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-5">
        <div className="page-header">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="page-title">Eye Exam (Ophthalmology)</h1>
              <p className="text-sm text-[var(--color-text-muted)]">Visual acuity, IOP, refraction, and assessment</p>
            </div>
          </div>
          {selectedPatient && (
        ...
        <div className="card p-4 relative">
          <label className="label">{t('dashboard.patient')}</label>
          ...
          <div className="card overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="p-12 flex flex-col items-center justify-center text-[var(--color-text-muted)]">
                ...
                <p className="text-sm font-medium">No prior eye exams</p>
                  ...
                  <tbody>
                    {exams.map((exam) => (
                      <tr key={exam.EyeExamId}>
                        <td className="font-data">{new Date(exam.ExamDate).toLocaleDateString()}</td>
        ...
        {!selectedPatient && (
          <div className="card p-12 flex flex-col items-center justify-center text-[var(--color-text-muted)] min-h-64">
            ...
            <p className="text-sm mt-1">Search and select a patient to record eye exams.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--color-card)] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden">
            ...
            <div className="flex border-b border-[var(--color-border)] px-4 flex-shrink-0 overflow-x-auto">
              {[
                { id: 'general', label: 'History' },
                { id: 'acuity', label: 'Visual Acuity' },
                { id: 'refraction', label: 'Refraction' },
                { id: 'antseg', label: 'Ant. Segment / IOP' },
                { id: 'fundus', label: 'Fundus' },
                { id: 'assessment', label: 'A/P' },
              ].map(t => (
                <button
                  key={t.id} type="button"
                  onClick={() => setActiveTab(t.id as any)}
                  className={`px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${activeTab === t.id ? 'border-blue-500 text-blue-600' : 'border-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text)]'}`}
                >
                  {t.label}
                </button>
              ))}
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <h4 className="font-semibold text-red-600 bg-red-50 p-2 rounded text-center border border-red-100">Right Eye (OD)</h4>
                      <div>
                        <label className="label text-xs">Distance (SC)</label>
                        <input className="input w-full" placeholder={t("dashboard.eg_2020")} value={form.SCODVA} onChange={e => setField('SCODVA', e.target.value)} />
                      </div>
                      <div>
                        <label className="label text-xs">Pinhole (PH)</label>
                        <input className="input w-full" placeholder={t("dashboard.eg_2020")} value={form.PHODVA} onChange={e => setField('PHODVA', e.target.value)} />
                      </div>
                    </div>
                    <div className="space-y-3">
                      <h4 className="font-semibold text-emerald-600 bg-emerald-50 p-2 rounded text-center border border-emerald-100">Left Eye (OS)</h4>
                      <div>
                        <label className="label text-xs">Distance (SC)</label>
                        <input className="input w-full" placeholder={t("dashboard.eg_2020")} value={form.SCOSVA} onChange={e => setField('SCOSVA', e.target.value)} />
                      </div>
                      <div>
                        <label className="label text-xs">Pinhole (PH)</label>
                        <input className="input w-full" placeholder={t("dashboard.eg_2020")} value={form.PHOSVA} onChange={e => setField('PHOSVA', e.target.value)} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3 p-4 border rounded-xl bg-red-50/30">
                      <h4 className="font-semibold text-red-600 text-sm">Right Eye (OD)</h4>
                    ...
                    <div className="space-y-3 p-4 border rounded-xl bg-emerald-50/30">
                      <h4 className="font-semibold text-emerald-600 text-sm">Left Eye (OS)</h4>
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <label className="label text-xs">SPH</label>
              ...
              {activeTab === 'antseg' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                       <h4 className="font-semibold text-red-600 text-sm">Right Eye (OD)</h4>
                       <div><label className="label text-xs">Cornea</label><input className="input w-full" value={form.ODCORNEA} onChange={e=>setField('ODCORNEA', e.target.value)} /></div>
                       <div><label className="label text-xs">Lens</label><input className="input w-full" value={form.ODLENS} onChange={e=>setField('ODLENS', e.target.value)} /></div>
                       <div><label className="label text-xs">IOP (NCT)</label><input className="input w-full" value={form.ODIOPNCT} onChange={e=>setField('ODIOPNCT', e.target.value)} /></div>
                    </div>
                    <div className="space-y-3">
                       <h4 className="font-semibold text-emerald-600 text-sm">Left Eye (OS)</h4>
                       <div><label className="label text-xs">Cornea</label><input className="input w-full" value={form.OSCORNEA} onChange={e=>setField('OSCORNEA', e.target.value)} /></div>
                       <div><label className="label text-xs">Lens</label><input className="input w-full" value={form.OSLENS} onChange={e=>setField('OSLENS', e.target.value)} /></div>
                       <div><label className="label text-xs">IOP (NCT)</label><input className="input w-full" value={form.OSIOPNCT} onChange={e=>setField('OSIOPNCT', e.target.value)} /></div>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'fundus' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                       <h4 className="font-semibold text-red-600 text-sm">Right Eye (OD)</h4>
                       <div><label className="label text-xs">Macula</label><input className="input w-full" value={form.ODMACULA} onChange={e=>setField('ODMACULA', e.target.value)} /></div>
                       <div><label className="label text-xs">Disc / CDR</label><input className="input w-full" value={form.ODDISC} onChange={e=>setField('ODDISC', e.target.value)} /></div>
                    </div>
                    <div className="space-y-3">
                       <h4 className="font-semibold text-emerald-600 text-sm">Left Eye (OS)</h4>
                       <div><label className="label text-xs">Macula</label><input className="input w-full" value={form.OSMACULA} onChange={e=>setField('OSMACULA', e.target.value)} /></div>
                       <div><label className="label text-xs">Disc / CDR</label><input className="input w-full" value={form.OSDISC} onChange={e=>setField('OSDISC', e.target.value)} /></div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="p-4 border-t border-[var(--color-border)] flex justify-between shrink-0 bg-[var(--color-bg-secondary)]">
              <div>
                <p className="text-xs text-[var(--color-text-muted)] mt-2">Make sure all relevant sections are filled before saving.</p>
              </div>
