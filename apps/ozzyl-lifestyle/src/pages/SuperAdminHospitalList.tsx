...
export default function SuperAdminHospitalList() {
  ...
  const fetchHospitals = async () => {
    ...
    try {
      ...
      toast.error(t('super-admin.failed_to_load_hospitals'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (hospital: Hospital) => {
    ...
    try {
      ...
      toast.error(t('super-admin.failed_to_update_hospital'));
    }
  };
            ...
            <div className="card max-w-md w-full mx-4 p-6">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  ...
                  <input name="name" required className="input-field w-full" placeholder={t("superAdmin.hospitalNameExample")} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">{t('super-admin:subdomain')}</label>
                  <input name="subdomain" required className="input-field w-full" placeholder={t("superAdmin.subdomainExample")} pattern="^[a-z0-9][a-z0-9-]*[a-z0-9]$" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">{t('super-admin:adminName')}</label>
                  <input name="adminName" required className="input-field w-full" placeholder={t("superAdmin.adminFullNamePlaceholder")} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">{t('super-admin:adminEmail')}</label>
                  <input name="adminEmail" type="email" required className="input-field w-full" placeholder={t("superAdmin.adminEmailPlaceholder")} />
                </div>
                <div>
