  ...
  const loadScan = useCallback(async () => {
    ...
    try {
      const [dRes, sRes] = await Promise.all([
        axios.get('/api/patient-duplicates/scan', { params: { method }, headers: authHeader() }),
        axios.get('/api/patient-duplicates/stats', { headers: authHeader() }),
      ]);
      setPairs(dRes.data?.data ?? []); setStats(sRes.data ?? null);
    } catch { setPairs([]); } finally { setLoading(false); }
  }, [method]);

  const loadHistory = useCallback(async () => {
    setLoading(true);
    try { const { data } = await axios.get('/api/patient-duplicates/history', { headers: authHeader() }); setHistory(data?.data ?? []); }
    catch { setHistory([]); } finally { setLoading(false); }
  }, []);

  useEffect(() => { if (tab === 'scan') loadScan(); else loadHistory(); }, [tab, loadScan, loadHistory]);

  const comparePair = async (id1: number, id2: number) => {
    try {
      const { data } = await axios.get('/api/patient-duplicates/compare', { params: { id1, id2 }, headers: authHeader() });
      setCompare(data); setMergeReason('');
    } catch { toast.error(t('patients.failed_to_load_comparison')); }
  };

  const doMerge = async (primaryId: number, secondaryId: number) => {
    if (!mergeReason) { toast.error(t('patients.merge_reason_required')); return; }
    setMerging(true);
    try {
      const { data } = await axios.post('/api/patient-duplicates/merge', { primary_id: primaryId, secondary_id: secondaryId, merge_reason: mergeReason }, { headers: authHeader() });
      toast.success(t('patients.recordsMerged', { count: data.total_records_moved }));
      setCompare(null); loadScan();
    } catch (err) { toast.error(axios.isAxiosError(err) ? (err.response?.data?.message ?? t('patientDuplicates.mergeFailed')) : t('patientDuplicates.failed')); }
        ...
        {tab === 'scan' && (
          <div className="space-y-4">
            <div className="flex gap-3 items-end">
              ...
              <button onClick={loadScan} className="btn-primary"><Search className="w-4 h-4" /> Scan</button>
            </div>
            ...
            : <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base"><thead><tr><th>{t('patientDuplicates.patientA')}</th><th>{t('patientDuplicates.patientB')}</th><th>{t('patientDuplicates.match')}</th><th>{t('patientDuplicates.confidence')}</th><th></th></tr></thead><tbody>
              {pairs.map((p, i) => (
                <tr key={i}>
                  ...
                  <td><button onClick={() => comparePair(p.id1, p.id2)} className="btn-ghost text-xs p-1"><Eye className="w-3.5 h-3.5" /> Compare</button></td>
                </tr>
              ))}
            </tbody></table></div></div>}
          </div>
