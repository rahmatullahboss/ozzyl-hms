...
export default function ImportExternalRecords({ role }: { role?: string }) {
  ...
  const handleNidLookup = async () => {
    ...
    try {
      const { data } = await axios.get(`/api/health-record/lookup?national_id=${nidInput}`, { headers: authHeader() });
      setLookupResult(data);
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Lookup failed';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleTokenLookup = async () => {
    ...
    try {
      const { data } = await axios.get(`/api/health-record/summary/${tokenInput}`);
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">বাহ্যিক স্বাস্থ্য রেকর্ড আমদানি</p>
          </div>
        </div>
