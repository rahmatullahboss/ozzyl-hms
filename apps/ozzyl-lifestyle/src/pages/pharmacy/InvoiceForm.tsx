  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const line of lines) {
      ...
      if (qty > line.availableQty) {
        toast.error(t('invoice.qtyExceeds', { defaultValue: `${line.itemName}: qty (${qty}) exceeds available stock (${line.availableQty})` }));
        return;
      }
    }
    ...
    if (Math.abs((paidAmt + creditAmt + depositDeducted) - subtotal) > 0.01) {
      toast.error(t('invoice.paymentMismatch', { defaultValue: `Payment mismatch: paid (${paidAmt}) + credit (${creditAmt}) + deposit (${depositDeducted}) must equal total (${subtotal.toFixed(2)})` }));
      return;
    }
    setSaving(true);
    try {
