...
export default function EPrescribingDashboard({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const submitFormulary = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AxiosError ? err.response?.data?.error : t('toast.error.addDrugFailed', { defaultValue: 'Failed to add drug' });
      toast.error(msg || t('toast.error.addDrugFailed', { defaultValue: 'Failed to add drug' }));
    }
  };
  ...
  const submitInteraction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AxiosError ? err.response?.data?.message : t('toast.error.addInteractionFailed', { defaultValue: 'Failed to add interaction' });
      toast.error(msg || t('toast.error.addInteractionFailed', { defaultValue: 'Failed to add interaction' }));
    }
  };
  ...
  const deleteInteraction = async (id: number) => {
    try {
      ...
      fetchInteractions();
    } catch { toast.error(t('toast.error.removeFailed', { defaultValue: 'Failed to remove' })); }
  };
  ...
  const runSafetyCheck = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else toast(t('toast.warning.safetyWarnings', { defaultValue: '⚠️ Safety warnings detected — please review' }), { icon: '🔍' });
    } catch (err) {
      const msg = err instanceof AxiosError ? err.response?.data?.error : t('toast.error.safetyCheckFailed', { defaultValue: 'Safety check failed' });
      toast.error(msg || t('toast.error.safetyCheckFailed', { defaultValue: 'Safety check failed' }));
    }
    setChecking(false);
  };
    ...
    <DashboardLayout role={role}>
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard title={t('kpi.drugCatalog', { defaultValue: 'Drug Catalog' })} value={stats.formulary_items} loading={loading} icon={<BookOpen className="w-5 h-5"/>} iconBg="bg-[var(--color-primary-light)] text-[var(--color-primary)]" />
          <KPICard title={t('kpi.interactionPairs', { defaultValue: 'Interaction Pairs' })} value={stats.interaction_pairs} loading={loading} icon={<Activity className="w-5 h-5"/>} iconBg="bg-amber-50 text-amber-600" />
          <KPICard title={t('kpi.safetyChecksRun', { defaultValue: 'Safety Checks Run' })} value={stats.total_safety_checks} loading={loading} icon={<ShieldCheck className="w-5 h-5"/>} iconBg="bg-emerald-50 text-emerald-600" />
          <KPICard title={t('kpi.warningsDetected', { defaultValue: 'Warnings Detected' })} value={stats.checks_with_warnings} loading={loading} icon={<AlertTriangle className="w-5 h-5"/>} iconBg="bg-red-50 text-red-600" />
        </div>
