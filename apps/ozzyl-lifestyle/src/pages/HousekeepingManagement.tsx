...
import { Sparkles, Plus, X, RefreshCw, CheckCircle, AlertTriangle, Clock, MapPin } from 'lucide-react';
...
const TABS = ['tasks', 'complaints', 'areas'] as const;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [tRes, sRes, aRes] = await Promise.all([
        axios.get('/api/housekeeping/tasks', { params, headers: authHeader() }),
        axios.get('/api/housekeeping/stats', { headers: authHeader() }),
        axios.get('/api/housekeeping/areas', { headers: authHeader() }),
      ]);
      setItems(tRes.data?.data ?? []); setStats(sRes.data?.tasks ?? null); setAreas(aRes.data?.data ?? []);
    } catch { setItems([]); } finally { setLoading(false); }
  }, [date, statusFilter]);
  useEffect(() => { load(); }, [load]);

  const updateStatus = async (id: number, status: string) => {
    try { await axios.put(`/api/housekeeping/tasks/${id}/status`, { status }, { headers: authHeader() }); toast.success(t('taskUpdated', { status })); load(); } catch { toast.error(t('failed', { ns: 'common' })); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    try { await axios.post('/api/housekeeping/tasks', form, { headers: authHeader() }); toast.success(t('taskCreated')); setShowForm(false); load(); } catch { toast.error(t('failed', { ns: 'common' })); } finally { setSaving(false); }
  };
      ...
      {showForm && (
        <Modal title={t('newHousekeepingTask')} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('cancel', { ns: 'common' })}</button><button type="submit" disabled={saving} className="btn-primary">{saving ? t('saving') : t('create')}</button></div>
          </form>
        </Modal>
      )}
    </div>
...
function ComplaintsTab() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const { data } = await axios.get('/api/housekeeping/complaints', { headers: authHeader() }); setItems(data?.data ?? []); } catch { setItems([]); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);

  const updateStatus = async (id: number, status: string) => {
    try { await axios.put(`/api/housekeeping/complaints/${id}/status`, { status }, { headers: authHeader() }); toast.success(t('complaintUpdated', { status })); load(); } catch { toast.error(t('failed', { ns: 'common' })); }
  };

  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setSaving(true); try { await axios.post('/api/housekeeping/complaints', form, { headers: authHeader() }); toast.success(t('complaintRegistered')); setShowForm(false); load(); } catch { toast.error(t('failed', { ns: 'common' })); } finally { setSaving(false); } };
      ...
      {showForm && (
        <Modal title={t('registerComplaint')} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('cancel', { ns: 'common' })}</button><button type="submit" disabled={saving} className="btn-primary">{saving ? t('saving') : t('register')}</button></div>
          </form>
        </Modal>
      )}
    </div>
...
function AreasTab() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const { data } = await axios.get('/api/housekeeping/areas', { headers: authHeader() }); setItems(data?.data ?? []); } catch { setItems([]); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setSaving(true); try { await axios.post('/api/housekeeping/areas', form, { headers: authHeader() }); toast.success(t('areaAdded')); setShowForm(false); load(); } catch { toast.error(t('failed', { ns: 'common' })); } finally { setSaving(false); } };
  return (
    <div className="space-y-4">
      ...
      <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base"><thead><tr><th>{t('area')}</th><th>{t('type')}</th><th>{t('floor')}</th><th>{t('building')}</th><th></th></tr></thead><tbody>
        ...
        : items.map(a => (<tr key={a.id}><td className="font-medium">{a.area_name}</td><td className="text-xs"><span className="badge-neutral">{a.area_type}</span></td><td className="text-sm">{a.floor ?? '—'}</td><td className="text-sm">{a.building ?? '—'}</td><td><button onClick={async () => { await axios.delete(`/api/housekeeping/areas/${a.id}`, { headers: authHeader() }); load(); }} className="btn-ghost text-xs text-red-400 p-1"><X className="w-3.5 h-3.5" /></button></td></tr>))}
      </tbody></table></div></div>
      {showForm && (<Modal title={t('addAreaTitle')} onClose={() => setShowForm(false)}><form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('cancel', { ns: 'common' })}</button><button type="submit" disabled={saving} className="btn-primary">{saving ? t('saving') : t('addArea')}</button></div>
      </form></Modal>)}
    </div>
  );
}
  ...
  return (
    <DashboardLayout role={role ?? 'hospital_admin'}>
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        {tab === 'tasks' && <TasksTab />}{tab === 'complaints' && <ComplaintsTab />}{tab === 'areas' && <AreasTab />}
      </div>
    </DashboardLayout>
  );
}
