...
function ActiveTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [aRes, sRes] = await Promise.all([axios.get('/api/ambulance/active', { headers: authHeader() }), axios.get('/api/ambulance/stats', { headers: authHeader() })]);
      setTrips(aRes.data?.data ?? []); setStats(sRes.data ?? null);
    } catch { setTrips([]); } finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); const iv = setInterval(load, 30000); return () => clearInterval(iv); }, [load]);

  const advance = async (id: number, status: string) => {
    try { await axios.put(`/api/ambulance/trips/${id}/status`, { status }, { headers: authHeader() }); toast.success(t('statusAdvanced', { status: status.replace('_', ' ') })); load(); } catch { toast.error(t('failed', { ns: 'common' })); }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end"><button onClick={load} className="btn-ghost"><RefreshCw className="w-4 h-4" /> {t('refresh', { ns: 'common' })}</button></div>
      ...
      : <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{trips.map((trip) => {
        ...
        return (
          <div key={trip.id} className={`card p-4 border-l-4 ${trip.urgency === 'emergency' ? 'border-l-red-500' : trip.urgency === 'urgent' ? 'border-l-amber-400' : 'border-l-blue-400'}`}>
            ...
            {next && <button onClick={() => advance(trip.id, next)} className="btn-primary text-xs mt-3 w-full">→ {next.replace('_', ' ')}</button>}
          </div>
        );
      })}</div>}
    </div>
...
function TripsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [tRes, vRes] = await Promise.all([axios.get('/api/ambulance/trips', { headers: authHeader() }), axios.get('/api/ambulance/vehicles', { headers: authHeader() })]);
      setItems(tRes.data?.data ?? []); setVehicles(vRes.data?.data ?? []);
    } catch { setItems([]); } finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!form.vehicle_id) { toast.error(t('selectVehicle', { ns: 'ambulance' })); return; }
    setSaving(true);
    try { await axios.post('/api/ambulance/trips', { ...form, vehicle_id: Number(form.vehicle_id) }, { headers: authHeader() }); toast.success(t('tripDispatched')); setShowForm(false); load(); } catch { toast.error(t('failed', { ns: 'common' })); } finally { setSaving(false); }
  };
      ...
      {showForm && (
        <Modal title={t('dispatchAmbulance')} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('cancel', { ns: 'common' })}</button><button type="submit" disabled={saving} className="btn-primary">{saving ? t('dispatching') : t('dispatch')}</button></div>
          </form>
        </Modal>
      )}
    </div>
...
function VehiclesTab() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const { data } = await axios.get('/api/ambulance/vehicles', { headers: authHeader() }); setItems(data?.data ?? []); } catch { setItems([]); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setSaving(true); try { await axios.post('/api/ambulance/vehicles', form, { headers: authHeader() }); toast.success(t('vehicleAdded')); setShowForm(false); load(); } catch { toast.error(t('failed', { ns: 'common' })); } finally { setSaving(false); } };
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <Modal title={t('addVehicleTitle')} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setShowForm(false)} className="btn-secondary">{t('cancel', { ns: 'common' })}</button><button type="submit" disabled={saving} className="btn-primary">{saving ? t('saving') : t('add')}</button></div>
          </form>
        </Modal>
      )}
    </div>
