...
export default function Camos({ role }: { role?: string }) {
  ...
  // ── Category browser ────────────────────────────────────────────────────
  const [categories, setCategories]           = useState<CamosCategory[]>([]);
  const [catLoading, setCatLoading]           = useState(false);
  const [selectedCat, setSelectedCat]         = useState<CamosCategory | null>(null);
  const [subcategories, setSubcategories]     = useState<CamosSubcategory[]>([]);
  const [selectedSub, setSelectedSub]         = useState<CamosSubcategory | null>(null);
  const [items, setItems]                     = useState<CamosItem[]>([]);
  ...
  useEffect(() => {
    setCatLoading(true);
    axios
      .get('/api/camos/categories', { headers: authHeaders() })
      .then(r => setCategories(r.data.Results ?? r.data ?? []))
      .catch(() => toast.error(t('toast.loadCategoriesFailed', { ns: 'camos', defaultValue: 'Failed to load categories' })))
      .finally(() => setCatLoading(false));
  }, []);

  // ── Load subcategories when category selected ───────────────────────────
  useEffect(() => {
    if (!selectedCat) { setSubcategories([]); setSelectedSub(null); setItems([]); return; }
    axios
      .get(`/api/camos/subcategories?categoryId=${selectedCat.CategoryId}`, {
        headers: authHeaders(),
      })
      .then(r => setSubcategories(r.data.Results ?? r.data ?? []))
      .catch(() => toast.error(t('toast.loadSubcategoriesFailed', { ns: 'camos', defaultValue: 'Failed to load subcategories' })));
  }, [selectedCat]);

  // ── Load items when subcategory selected ───────────────────────────────
  useEffect(() => {
    if (!selectedSub) { setItems([]); return; }
    setItemsLoading(true);
    axios
      .get(`/api/camos/items?subcategoryId=${selectedSub.SubcategoryId}`, {
        headers: authHeaders(),
      })
      .then(r => setItems(r.data.Results ?? r.data ?? []))
      .catch(() => toast.error(t('toast.loadItemsFailed', { ns: 'camos', defaultValue: 'Failed to load assessment items' })))
  ...
  const loadHistory = useCallback(async (patientId: number) => {
    setHistLoading(true);
    try {
      const r = await axios.get(`/api/camos/assessments?patientId=${patientId}`, {
        headers: authHeaders(),
      });
      setPastAssessments(r.data.Results ?? r.data ?? []);
    } catch {
      toast.error(t('toast.loadHistoryFailed', { ns: 'camos', defaultValue: 'Failed to load assessment history' }));
    } finally {
            ...
            <div className="card p-4">
              ...
              {selectedPatient ? (
                <div className="flex items-center gap-3 p-3 rounded-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border)]">
                  ...
                  <button
                    onClick={() => { setSelectedPatient(null); setPatientQuery(''); setPastAssessments([]); }}
              ...
              <div className="card p-5 space-y-5">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={submitting || !selectedPatient || !assessmentTitle}
                    className="btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <CheckCircle2 className="w-4 h-4" />
                    {submitting ? t('common.saving', { defaultValue: 'Saving…' }) : t('submitAssessment', { ns: 'camos', defaultValue: 'Submit Assessment' })}
                  </button>
                </div>
              </div>
            )}
