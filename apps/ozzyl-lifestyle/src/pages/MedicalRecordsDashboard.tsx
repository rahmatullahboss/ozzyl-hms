...
export default function MedicalRecordsDashboard({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const submitRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AxiosError ? err.response?.data?.error : t('toast.recordCreateFailed');
      toast.error(msg || t('toast.recordCreateFailed'));
    }
    setSubmitting(false);
  };
  ...
  const submitBirth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AxiosError ? err.response?.data?.error : t('toast.birthRecordFailed');
      toast.error(msg || t('toast.birthRecordFailed'));
    }
    setSubmitting(false);
  };
  ...
  const submitDeath = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof AxiosError ? err.response?.data?.error : t('toast.deathRecordFailed');
      toast.error(msg || t('toast.deathRecordFailed'));
    }
    setSubmitting(false);
  };
      ...
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        <div className="page-header">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={fetchStats} className="btn-secondary" title={t('actions.refresh')}>
          ...
          <div className="space-y-4">
            <div className="card p-4 flex flex-wrap gap-3 items-center justify-between">
              ...
              <button onClick={() => setShowRecordModal(true)} className="btn-primary">
                <Plus className="w-4 h-4" /> {t('actions.newRecord')}
              </button>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="table-base">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>{t('records.patient')}</th>
                      <th>{t('records.fileNo')}</th>
                      <th>{t('records.dischargeType')}</th>
                      <th>{t('records.operation')}</th>
                      <th>{t('records.referredTo')}</th>
                      <th>{t('records.created')}</th>
                  ...
                  <tbody>
                    {recordsLoading ? (
                      ...
                      <tr><td colSpan={8} className="py-16 text-center text-[var(--color-text-muted)]">{t('records.noRecordsFound')}</td></tr>
                    ) : (
                      records.map((r, idx) => (
                        <tr key={r.id}>
                          ...
                          <td>
                            <button className="btn-ghost p-1.5" title={t('records.viewDetails')}>
          ...
          <div className="space-y-4">
            <div className="card p-4 flex flex-wrap gap-3 items-center justify-between">
              <div className="flex gap-2 items-center">
                ...
                <button onClick={fetchBirths} className="btn-secondary">{t('actions.apply')}</button>
              </div>
              <button onClick={() => setShowBirthModal(true)} className="btn-primary">
                <Plus className="w-4 h-4" /> {t('actions.registerBirth')}
              </button>
            </div>
            ...
            <div className="card overflow-hidden">
              <div className="p-4 border-b border-[var(--color-border)] flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm">{t('birth.registerTitle', { count: birthTotal })}</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="table-base">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>{t('birth.certificateNo')}</th>
                      <th>{t('birth.babyName')}</th>
                      <th>{t('birth.sex')}</th>
                      <th>{t('birth.weightKg')}</th>
                      <th>{t('birth.birthDate')}</th>
                      <th>{t('birth.deliveryType')}</th>
                      <th>{t('birth.mother')}</th>
                      <th>{t('birth.father')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {birthsLoading ? (
                      ...
                      <tr><td colSpan={9} className="py-16 text-center text-[var(--color-text-muted)]">{t('birth.noRecordsFound')}</td></tr>
                    ) : (
                      births.map((b, idx) => (
                        <tr key={b.id}>
                          <td className="text-[var(--color-text-muted)]">{idx + 1}</td>
                          ...
                          <td>
                            {b.sex ? (
                              <span className={`badge ${b.sex === 'Male' ? 'badge-info' : 'badge-warning'}`}>{t(`birth.sexValues.${b.sex.toLowerCase()}`, b.sex)}</span>
                            ) : '—'}
                          </td>
          ...
          <div className="space-y-4">
            <div className="card p-4 flex flex-wrap gap-3 items-center justify-between">
              <div className="flex gap-2 items-center">
                ...
                <button onClick={fetchDeaths} className="btn-secondary">{t('actions.apply')}</button>
              </div>
              <button onClick={() => setShowDeathModal(true)} className="btn-primary">
                <Plus className="w-4 h-4" /> {t('actions.registerDeath')}
              </button>
            </div>
            ...
            <div className="card overflow-hidden">
              <div className="p-4 border-b border-[var(--color-border)] flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm">{t('death.registerTitle', { count: deathTotal })}</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="table-base">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>{t('death.certificateNo')}</th>
                      <th>{t('death.patient')}</th>
                      <th>{t('death.dateOfDeath')}</th>
                      <th>{t('death.time')}</th>
                      <th>{t('death.causeOfDeath')}</th>
                      <th>{t('death.manner')}</th>
                      <th>{t('death.place')}</th>
                      <th>{t('death.age')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {deathsLoading ? (
                      ...
                      <tr><td colSpan={9} className="py-16 text-center text-[var(--color-text-muted)]">{t('death.noRecordsFound')}</td></tr>
                    ) : (
                      deaths.map((d, idx) => (
                        <tr key={d.id}>
                          <td className="text-[var(--color-text-muted)]">{idx + 1}</td>
            ...
            <div className="card p-4">
              <div className="flex gap-3 items-center">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder={t('placeholders.searchIcd10')}
                    value={icdSearch}
              ...
              <p className="text-xs text-[var(--color-text-muted)] mt-2">
                {t('icd10.showingCodes', { count: icdCodes.length })}
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="table-base">
                  <thead>
                    <tr>
                      <th>{t('icd10.code')}</th>
                      <th>{t('icd10.description')}</th>
                      <th>{t('icd10.groupId')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {icdLoading ? (
                      ...
                      <tr><td colSpan={3} className="py-16 text-center text-[var(--color-text-muted)]">{icdSearch ? t('icd10.noCodesFoundWithSearch') : t('icd10.noCodesFound')}</td></tr>
                    ) : (
                      icdCodes.map(code => (
                        <tr key={code.id}>
          ...
          <div className="space-y-4">
            <div className="card overflow-hidden">
              <div className="p-4 border-b border-[var(--color-border)] flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm">{t('referrals.title')}</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="table-base">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>{t('referrals.patient')}</th>
                      <th>{t('referrals.referredTo')}</th>
                      <th>{t('referrals.referredDate')}</th>
                      <th>{t('referrals.fileNo')}</th>
                      <th>{t('referrals.reason')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {referralsLoading ? (
                      ...
                      <tr><td colSpan={6} className="py-16 text-center text-[var(--color-text-muted)]">{t('referrals.noRecordsFound')}</td></tr>
                    ) : (
                      referrals.map((r, idx) => (
                        <tr key={r.id}>
                          <td className="text-[var(--color-text-muted)]">{idx + 1}</td>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={submitRecord} className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('modals.newRecord.patientId')} *</label>
                    <input className="input" type="number" required value={recordForm.patient_id} onChange={e => setRecordForm({ ...recordForm, patient_id: e.target.value })} placeholder={t('placeholders.patientId')} />
                  </div>
                  <div>
                    <label className="label">{t('modals.newRecord.fileNumber')}</label>
                    <input className="input" value={recordForm.file_number} onChange={e => setRecordForm({ ...recordForm, file_number: e.target.value })} placeholder={t('placeholders.fileNumber')} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('modals.newRecord.dischargeType')}</label>
                    <select className="input" value={recordForm.discharge_type} onChange={e => setRecordForm({ ...recordForm, discharge_type: e.target.value })}>
                      ...
                      <option value="normal">{t('dischargeTypes.normal')}</option>
                      <option value="lama">{t('dischargeTypes.lama')}</option>
                      <option value="absconded">{t('dischargeTypes.absconded')}</option>
                      <option value="referred">{t('dischargeTypes.referred')}</option>
                      <option value="expired">{t('dischargeTypes.expired')}</option>
                    </select>
                  </div>
                  <div>
                    <label className="label">{t('modals.newRecord.dischargeCondition')}</label>
                    <select className="input" value={recordForm.discharge_condition} onChange={e => setRecordForm({ ...recordForm, discharge_condition: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="improved">{t('dischargeConditions.improved')}</option>
                      <option value="unchanged">{t('dischargeConditions.unchanged')}</option>
                      <option value="worsened">{t('dischargeConditions.worsened')}</option>
                      <option value="cured">{t('dischargeConditions.cured')}</option>
                    </select>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="op_conducted" className="label cursor-pointer">{t('modals.newRecord.operationConducted')}</label>
                </div>
                {recordForm.is_operation_conducted && (
                  <div className="grid grid-cols-2 gap-4 pl-6 border-l-2 border-[var(--color-border)]">
                    <div>
                      <label className="label">{t('modals.newRecord.operationDate')}</label>
                    ...
                    <div>
                      <label className="label">{t('modals.newRecord.operationDiagnosis')}</label>
                      <input className="input" value={recordForm.operation_diagnosis} onChange={e => setRecordForm({ ...recordForm, operation_diagnosis: e.target.value })} placeholder={t('placeholders.operationDiagnosis')} />
                    </div>
                  </div>
                )}
                {recordForm.discharge_type === 'referred' && (
                  <div className="grid grid-cols-2 gap-4 pl-6 border-l-2 border-amber-200">
                    <div>
                      <label className="label">{t('modals.newRecord.referredTo')}</label>
                      <input className="input" value={recordForm.referred_to} onChange={e => setRecordForm({ ...recordForm, referred_to: e.target.value })} placeholder={t('placeholders.referredTo')} />
                    </div>
                    <div>
                      <label className="label">{t('modals.newRecord.referralDate')}</label>
                    ...
                    <div className="col-span-2">
                      <label className="label">{t('modals.newRecord.referralReason')}</label>
                      <textarea className="input min-h-[60px]" value={recordForm.referred_reason} onChange={e => setRecordForm({ ...recordForm, referred_reason: e.target.value })} placeholder={t('placeholders.referralReason')} />
                    </div>
                  </div>
                )}
                <div>
                  <label className="label">{t('modals.newRecord.remarks')}</label>
                  <textarea className="input min-h-[60px]" value={recordForm.remarks} onChange={e => setRecordForm({ ...recordForm, remarks: e.target.value })} placeholder={t('placeholders.remarks')} />
                </div>
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={() => setShowRecordModal(false)} className="btn-secondary">{t('common.cancel')}</button>
                  <button type="submit" disabled={submitting} className="btn-primary">{submitting ? t('common.creating') : t('modals.newRecord.createRecord')}</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {showBirthModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between p-5 border-b border-[var(--color-border)]">
                <h3 className="font-semibold flex items-center gap-2"><Baby className="w-5 h-5 text-pink-500" /> {t('modals.birth.title')}</h3>
              ...
              <form onSubmit={submitBirth} className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('modals.birth.mPatientId')} *</label>
                    <input className="input" type="number" required value={birthForm.patient_id} onChange={e => setBirthForm({ ...birthForm, patient_id: e.target.value })} placeholder={t('placeholders.motherPatientId')} />
                  </div>
                  <div>
                    <label className="label">{t('modals.birth.babyName')}</label>
                    <input className="input" value={birthForm.baby_name} onChange={e => setBirthForm({ ...birthForm, baby_name: e.target.value })} placeholder={t('placeholders.optional')} />
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="label">{t('birth.sex')}</label>
                    <select className="input" value={birthForm.sex} onChange={e => setBirthForm({ ...birthForm, sex: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Male">{t('birth.sexValues.male')}</option>
                      <option value="Female">{t('birth.sexValues.female')}</option>
                      <option value="Other">{t('birth.sexValues.other')}</option>
                    </select>
                  </div>
                  <div>
                    <label className="label">{t('birth.weightKg')}</label>
                    <input className="input" type="number" step="0.01" min="0" value={birthForm.weight_kg} onChange={e => setBirthForm({ ...birthForm, weight_kg: e.target.value })} placeholder={t('placeholders.weightKg')} />
                  </div>
                  <div>
                    <label className="label">{t('birth.birthType')}</label>
                    <select className="input" value={birthForm.birth_type} onChange={e => setBirthForm({ ...birthForm, birth_type: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Single">{t('birth.birthTypes.single')}</option>
                      <option value="Twin">{t('birth.birthTypes.twin')}</option>
                      <option value="Triplet">{t('birth.birthTypes.triplet')}</option>
                      <option value="Quadruplet">{t('birth.birthTypes.quadruplet')}</option>
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('birth.birthDate')} *</label>
                  ...
                  <div>
                    <label className="label">{t('birth.birthTime')}</label>
                    <input type="time" className="input" value={birthForm.birth_time} onChange={e => setBirthForm({ ...birthForm, birth_time: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('birth.deliveryType')}</label>
                    <select className="input" value={birthForm.delivery_type} onChange={e => setBirthForm({ ...birthForm, delivery_type: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Normal">{t('birth.deliveryTypes.normal')}</option>
                      <option value="Cesarean">{t('birth.deliveryTypes.cesarean')}</option>
                      <option value="Forceps">{t('birth.deliveryTypes.forceps')}</option>
                      <option value="Vacuum">{t('birth.deliveryTypes.vacuum')}</option>
                    </select>
                  </div>
                  <div>
                    <label className="label">{t('birth.birthCondition')}</label>
                    <select className="input" value={birthForm.birth_condition} onChange={e => setBirthForm({ ...birthForm, birth_condition: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Alive">{t('birth.birthConditions.alive')}</option>
                      <option value="Stillborn">{t('birth.birthConditions.stillborn')}</option>
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('birth.fatherName')}</label>
                    <input className="input" value={birthForm.father_name} onChange={e => setBirthForm({ ...birthForm, father_name: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">{t('birth.motherName')}</label>
                    <input className="input" value={birthForm.mother_name} onChange={e => setBirthForm({ ...birthForm, mother_name: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('modals.birth.issuedBy')}</label>
                    <input className="input" value={birthForm.issued_by} onChange={e => setBirthForm({ ...birthForm, issued_by: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">{t('modals.birth.certifiedBy')}</label>
                    <input className="input" value={birthForm.certified_by} onChange={e => setBirthForm({ ...birthForm, certified_by: e.target.value })} />
                  </div>
                </div>
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={() => setShowBirthModal(false)} className="btn-secondary">{t('common.cancel')}</button>
                  <button type="submit" disabled={submitting} className="btn-primary">{submitting ? t('common.registering') : t('modals.birth.registerBirth')}</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {showDeathModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between p-5 border-b border-[var(--color-border)]">
                <h3 className="font-semibold flex items-center gap-2"><Heart className="w-5 h-5 text-slate-500" /> {t('modals.death.title')}</h3>
              ...
              <form onSubmit={submitDeath} className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('modals.death.patientId')} *</label>
                    <input className="input" type="number" required value={deathForm.patient_id} onChange={e => setDeathForm({ ...deathForm, patient_id: e.target.value })} placeholder={t('placeholders.patientId')} />
                  </div>
                  <div>
                    <label className="label">{t('death.ageAtDeath')}</label>
                    <input className="input" value={deathForm.age_at_death} onChange={e => setDeathForm({ ...deathForm, age_at_death: e.target.value })} placeholder={t('placeholders.ageAtDeath')} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('death.dateOfDeath')} *</label>
                  ...
                  <div>
                    <label className="label">{t('death.timeOfDeath')}</label>
                    <input type="time" className="input" value={deathForm.death_time} onChange={e => setDeathForm({ ...deathForm, death_time: e.target.value })} />
                  </div>
                </div>
                <div>
                  <label className="label">{t('death.causeOfDeath')}</label>
                  <textarea className="input min-h-[60px]" value={deathForm.cause_of_death} onChange={e => setDeathForm({ ...deathForm, cause_of_death: e.target.value })} placeholder={t('placeholders.primaryCause')} />
                </div>
                <div>
                  <label className="label">{t('death.secondaryCause')}</label>
                  <textarea className="input min-h-[60px]" value={deathForm.secondary_cause} onChange={e => setDeathForm({ ...deathForm, secondary_cause: e.target.value })} placeholder={t('placeholders.contributingCause')} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label">{t('death.mannerOfDeath')}</label>
                    <select className="input" value={deathForm.manner_of_death} onChange={e => setDeathForm({ ...deathForm, manner_of_death: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Natural">{t('death.mannerValues.natural')}</option>
                      <option value="Accident">{t('death.mannerValues.accident')}</option>
                      <option value="Suicide">{t('death.mannerValues.suicide')}</option>
                      <option value="Homicide">{t('death.mannerValues.homicide')}</option>
                      <option value="Undetermined">{t('death.mannerValues.undetermined')}</option>
                    </select>
                  </div>
                  <div>
                    <label className="label">{t('death.placeOfDeath')}</label>
                    <select className="input" value={deathForm.place_of_death} onChange={e => setDeathForm({ ...deathForm, place_of_death: e.target.value })}>
                      <option value="">{t('common.select')}</option>
                      <option value="Ward">{t('death.placeValues.ward')}</option>
                      <option value="ICU">{t('death.placeValues.icu')}</option>
                      <option value="Emergency">{t('death.placeValues.emergency')}</option>
                      <option value="OT">{t('death.placeValues.ot')}</option>
                      <option value="Other">{t('death.placeValues.other')}</option>
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="label">{t('death.fatherName')}</label>
                    <input className="input" value={deathForm.father_name} onChange={e => setDeathForm({ ...deathForm, father_name: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">{t('death.motherName')}</label>
                    <input className="input" value={deathForm.mother_name} onChange={e => setDeathForm({ ...deathForm, mother_name: e.target.value })} />
                  </div>
                  <div>
                    <label className="label">{t('death.spouseName')}</label>
                    <input className="input" value={deathForm.spouse_name} onChange={e => setDeathForm({ ...deathForm, spouse_name: e.target.value })} />
                  </div>
                </div>
                <div>
                  <label className="label">{t('modals.death.certifiedBy')}</label>
                  <input className="input" value={deathForm.certified_by} onChange={e => setDeathForm({ ...deathForm, certified_by: e.target.value })} placeholder={t('placeholders.doctorName')} />
                </div>
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={() => setShowDeathModal(false)} className="btn-secondary">{t('common.cancel')}</button>
                  <button type="submit" disabled={submitting} className="btn-primary">{submitting ? t('common.registering') : t('modals.death.registerDeath')}</button>
                </div>
              </form>
            </div>
          </div>
