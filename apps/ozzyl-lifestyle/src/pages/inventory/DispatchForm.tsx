...
export default function DispatchForm({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('inventory.dispatch_created')); navigate(`${base}/inventory/dispatches`);
    } catch (err: any) { toast.error(err?.response?.data?.error || t('inventory.createDispatchFailed')); }
