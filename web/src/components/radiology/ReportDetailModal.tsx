...
export default function ReportDetailModal({ reportId, onClose, onFinalized }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await apiFetch<{ report: ReportDetail }>(`/api/radiology/reports/${reportId}`);
  ...
  const handleFinalize = useCallback(async () => {
    ...
    setFinalizing(true);
    try {
      await apiFetch(`/api/radiology/reports/${report.id}/finalize`, { method: 'PATCH', body: {} });
              ...
              <>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-[var(--color-text)]">
                        {report.patient_name ?? t('orders.patientWithId', { id: report.patient_id })}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-[var(--color-border)]">
                  ...
                  <div className="flex items-center gap-1 text-xs font-mono text-sky-600">
                    ...
                    {report.radiology_number ?? t('reports.reportWithId', { id: report.id })}
                  </div>
                </div>
              </>
            ) : null}
