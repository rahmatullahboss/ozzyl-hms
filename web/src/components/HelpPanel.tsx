      ...
      >
        ...
        <div className="flex items-start justify-between p-5 border-b border-[var(--color-border)] bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-dark,#005fa3)] text-white rounded-none">
          ...
          <button
            ...
            aria-label={t('help.closePanel', { defaultValue: 'Close help panel' })}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-widest text-[var(--color-text-muted)] mb-3">
              {t('help.stepGuide', { defaultValue: 'Step-by-step guide' })}
            </h3>
          ...
          {content.tips && content.tips.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-widest text-[var(--color-text-muted)] mb-3 flex items-center gap-1.5">
                ...
                {t('help.proTips', { defaultValue: 'Pro tips' })}
              </h3>
        ...
        <div className="p-4 border-t border-[var(--color-border)] bg-[var(--color-surface-secondary,var(--color-surface))]">
          <p className="text-xs text-center text-[var(--color-text-muted)]">
            {t('help.contactAdmin', { defaultValue: 'For additional support, contact your system administrator' })}
          </p>
          {content.fullGuideLink && (
            <div className="mt-4">
              <a
                href={content.fullGuideLink}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark,#005fa3)] text-white rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow-md"
              >
                <BookOpen className="w-4 h-4" />
                {t('help.viewFullGuide', { defaultValue: 'View full guide' })}
              </a>
            </div>
          )}
        </div>
