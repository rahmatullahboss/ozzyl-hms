...
export default function PdfImportModal({ isOpen, onClose, onImportComplete }: PdfImportModalProps) {
  ...
  const handleOcrUpload = useCallback(async () => {
    ...
    try {
      ...
      // FormData upload — use fetch directly since apiFetch JSON-stringifies body
      const token = getAccessToken();
      const res = await fetch('/api/shareholders/ocr-pdf', {
        method: 'POST',
      ...
      const ocrData: { rawText: string; pageCount: number } = await res.json();
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message
        ?? (err instanceof Error ? err.message : t('shareholders.ocrFailed'));
      toast.error(msg);
    } finally {
      setOcrLoading(false);
    }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const responseData = await apiFetch<{ results: ImportResult[]; summary: { imported: number; failed: number } }>('/api/shareholders/bulk-import', {
        method: 'POST',
        body: { shareholders: shareholdersToImport, skipDuplicates },
      });

      setImportResults(responseData.results);
      setStep('result');

      const { imported, failed } = responseData.summary;
      if (imported > 0) {
      ...
      toast.error(error instanceof Error ? error.message : t('shareholders.importError'));
      setStep('preview');
    } finally {
      setImporting(false);
    }
