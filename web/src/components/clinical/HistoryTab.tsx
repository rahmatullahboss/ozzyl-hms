...
export default function HistoryTab({ patientId }: { patientId: number }) {
  ...
  const fetchHistory = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ history?: any[] }>(`/api/clinical/history/${activeSubTab}?patientId=${patientId}`);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`/api/clinical/history/${activeSubTab}`, {
        method: 'POST',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiFetch(`/api/clinical/history/${activeSubTab}/${id}`, { method: 'DELETE' });
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button onClick={fetchHistory} className="btn-ghost" title={t('common.refresh')} aria-label={t('common.refresh')}>
            ...
            {loading ? (
              ...
              records.map(r => (
                <tr key={r.id}>
                  ...
                  {activeSubTab === 'surgical' && <>
                    <td className="font-medium">{r.surgery_type}</td><td>{r.surgery_date ? formatDisplayDate(r.surgery_date) : '-'}</td><td>{r.icd10_code || '-'}</td><td>{r.note || '-'}</td>
                  </>}
