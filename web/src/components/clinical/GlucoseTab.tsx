...
export default function GlucoseTab({ patientId }: { patientId: number }) {
  ...
  const fetchReadings = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ glucose?: Glucose[] }>(`/api/clinical/glucose?patientId=${patientId}`);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiFetch('/api/clinical/glucose', {
        method: 'POST',
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiFetch(`/api/clinical/glucose/${id}`, { method: 'DELETE' });
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="btn-ghost focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]"
      ...
      {showAdd && (
        <div className="card p-4 border border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/50 dark:bg-indigo-900/10 mb-4">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label htmlFor="glucose-value" className="label">{t('glucose.value')} {t('common.required')}</label>
              <input id="glucose-value" type="number" step="0.1" required value={form.sugar_value} onChange={e => setForm({ ...form, sugar_value: e.target.value })} className="input" placeholder={t('placeholders.glucoseValue')} />
            </div>
            <div>
              <label htmlFor="glucose-unit" className="label">{t('glucose.unit')}</label>
              <select id="glucose-unit" value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })} className="input">
                <option value="mg/dL">{t('glucose.units.mgdl')}</option>
                <option value="mmol/L">{t('glucose.units.mmol')}</option>
              </select>
            </div>
            <div>
              <label htmlFor="glucose-type" className="label">{t('glucose.type')}</label>
              <select id="glucose-type" value={form.bsl_type} onChange={e => setForm({ ...form, bsl_type: e.target.value as any })} className="input">
            ...
            <div>
              <label htmlFor="glucose-time" className="label">{t('glucose.time')} {t('common.required')}</label>
              <input id="glucose-time" type="datetime-local" required value={form.measurement_time} onChange={e => setForm({ ...form, measurement_time: e.target.value })} className="input" />
            </div>
                ...
                <tr key={r.id}>
                  ...
                  <td className="text-right">
                    <button
                      ...
                      className="text-red-500 hover:bg-red-50 p-1.5 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]"
