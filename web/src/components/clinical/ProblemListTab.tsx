...
export default function ProblemListTab({ patientId }: { patientId: number }) {
  ...
  const fetchProblems = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ problems?: Problem[] }>(`/api/clinical/problems?patientId=${patientId}`);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button onClick={fetchProblems} className="btn-ghost" title={t('common.refresh')} aria-label={t('common.refresh')}>
            ...
            {loading ? (
              ...
              problems.map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.beg_date ? formatDisplayDate(p.beg_date) : '-'}</td>
