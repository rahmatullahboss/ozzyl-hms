...
export default function HealthRecordSharing({ role }: { role?: string }) {
  ...
  const fetchLinkedRecords = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await api.get<{ linked_records?: LinkedRecord[]; national_id?: string; uhid?: string }>(`/api/health-record/linked-records?patient_id=${patientId}`);
  ...
  const fetchConsents = useCallback(async () => {
    if (!patientId) return;
    try {
      const data = await api.get<{ consents?: Consent[] }>(`/api/health-record/consents?patient_id=${patientId}`);
  ...
  const setNidMutation = useApiMutation<unknown, { national_id: string }>(
    'put',
    `/api/patients/${patientId}/national-id`,
    {
      onSuccess: () => {
        ...
        setNidInput('');
        fetchLinkedRecords();
        queryClient.invalidateQueries({ queryKey: queryKeys.healthRecord.all });
      },
      onError: (err) => {
        toast.error(err.message || 'Failed to update NID');
      },
    },
  );
  ...
  const handleSetNid = async () => {
    ...
    setNidMutation.mutate({ national_id: nidInput });
  };

  const grantConsentMutation = useApiMutation<unknown, { consent_type: string; duration_hours: number }>(
    'post',
    `/api/health-record/consent?patient_id=${patientId}`,
    {
      onSuccess: () => {
  ...
  const handleGrantConsent = async () => {
    if (!patientId) return;
    grantConsentMutation.mutate({ consent_type: consentType, duration_hours: durationHours });
  };

  const handleRevokeConsent = async (consentId: number) => {
    try {
      await api.delete(`/api/health-record/consent/${consentId}`);
  ...
  const handleGenerateToken = async () => {
    if (!patientId) return;
    try {
      const data = await api.post<{ token: string }>(`/api/health-record/generate-token?patient_id=${patientId}`, {
        scope: tokenScope,
      ...
      const msg = (err as { message?: string })?.message ?? 'Failed to generate token';
      toast.error(msg);
    }
  };
  ...
  const handleEmergencyAccess = async () => {
    ...
    try {
      const data = await api.post<Record<string, unknown>>('/api/health-record/emergency-access', {
  ...
  const handleRedeemVisitPass = async () => {
    ...
    try {
      ...
      const data = await api.post<typeof visitPassResult>('/api/visit-pass/redeem', payload);
      setVisitPassResult(data);
      toast.success(t('patients.visit_pass_redeemed'));
    } catch (err: unknown) {
      const msg = (err as { message?: string })?.message ?? 'Failed to redeem visit pass';
      toast.error(msg);
    } finally {
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t('healthRecordSharing', { ns: 'staff' })}</h1>
              <p className="text-sm text-gray-500">{t('healthRecordSharingSubtitle', { ns: 'staff' })}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border p-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={t('enter_patient_id', { ns: 'staff' })}
            ...
            >
              <Search className="w-4 h-4" /> {t('load', { ns: 'staff' })}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border p-5 space-y-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                ...
                {t('redeemVisitPass', { ns: 'staff' })}
              </h2>
              <p className="text-sm text-gray-500 mt-1">
                {t('visitPassDescription', { ns: 'staff' })}
              </p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              placeholder={t("paste_token_or_enter_code_like_vpabc123", { ns: 'staff' })}
            ...
            >
              {visitPassLoading ? t('reredeeming', { ns: 'staff' }) : t('redeem', { ns: 'staff' })}
            </button>
          </div>
                ...
                <div className="space-y-2">
                  {linkedRecords.map((r, i) => (
                    <div key={i} className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-xs text-gray-400">{formatDisplayDate(r.linked_at)}</span>
                    </div>
                  ))}
                </div>
              )}
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-xs text-gray-400">
                            Expires: {formatDisplayDate(c.expires_at)}
                          </span>
