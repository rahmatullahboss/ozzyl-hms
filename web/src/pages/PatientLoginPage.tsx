...
function getPhoneError(phone: string, t: (key: string) => string) {
  if (!phone) return '';
  if (!phonePattern.test(phone)) return t('patientLogin.phoneFormatError');
  return '';
}

function getNidError(nid: string, t: (key: string) => string) {
  if (!nid) return '';
  if (!nidPattern.test(nid)) return t('patientLogin.nidFormatError');
  return '';
}
...
export default function PatientLoginPage() {
  ...
  const phoneError = getPhoneError(phone, t);
  const nidError = getNidError(nationalId, t);
  ...
  const registerProgress = [
    ...
    Boolean(confirmPassword) && !confirmPasswordError,
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-slate-950">
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-6 sm:px-12 lg:px-16 xl:px-24 py-12 bg-white dark:bg-slate-900">
        <div className="mb-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <img src="/ozzyl-logo.svg" alt="Ozzyl" className="w-10 h-10 rounded-xl shadow-md shadow-cyan-500/20" />
              <div>
        ...
        <div className="max-w-md mb-5 flex rounded-2xl bg-slate-100 dark:bg-slate-800 p-1">
          {(['login', 'register', 'forgot'] as AuthTab[]).map((tab) => (
            <button
              ...
              className={`flex-1 rounded-xl px-3 py-2 text-sm font-semibold transition ${
                activeTab === tab
                  ? 'bg-white dark:bg-slate-950 text-cyan-700 dark:text-cyan-300 shadow-sm'
                  : 'text-slate-500 dark:text-slate-400'
              }`}
            >
              {tab === 'login' ? t('patientLogin.loginTab') : tab === 'register' ? t('patientLogin.registerTab') : t('patientLogin.forgotTab')}
            </button>
          ))}
        </div>
        ...
        <div className="absolute -top-20 -right-10 w-72 h-72 rounded-full bg-cyan-300/20 blur-3xl" />
        <div className="absolute -bottom-24 -left-10 w-80 h-80 rounded-full bg-teal-300/20 blur-3xl" />

        <div className="relative z-10 max-w-lg px-10">
          <div className="rounded-3xl border border-white/60 bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl p-8 shadow-2xl shadow-cyan-900/10">
            ...
            <div className="mt-6 grid gap-3">
              <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800 px-4 py-3 bg-white/70 dark:bg-slate-950/40">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Global identity first</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Each patient gets one universal account and one global health identity.</p>
              </div>
              <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800 px-4 py-3 bg-white/70 dark:bg-slate-950/40">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Profile can mature later</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Missing phone or NID can be added after Google signup from the dashboard.</p>
              </div>
              <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800 px-4 py-3 bg-white/70 dark:bg-slate-950/40">
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">What you unlock immediately</p>
                <ul className="mt-2 space-y-2 text-xs text-slate-500 dark:text-slate-400">
                  <li>Health card and portable UHID</li>
                  <li>Prescriptions, reports, and visit history in one place</li>
                  <li>Vault uploads and self-reported health data</li>
                </ul>
              </div>
            </div>
            <div className="mt-6 flex flex-col gap-4">
              ...
              >
                {t('patientLogin.goToDashboard')}
                <ArrowRight className="w-4 h-4" />
              </Link>
              {!Capacitor.isNativePlatform() && (
                <a
                  href="/api/downloads/android"
                  className="inline-flex max-w-fit items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-md hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
                  download
                >
                  <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M17.525 9.774l1.9-3.284a.434.434 0 0 0-.158-.593.435.435 0 0 0-.594.157l-1.936 3.344A10.871 10.871 0 0 0 12 8.423c-1.636 0-3.176.36-4.542.996L5.522 6.074a.435.435 0 0 0-.594-.157.434.434 0 0 0-.158.593l1.898 3.282C3.125 11.597 1.056 14.509 1 18h22c-.056-3.489-2.124-6.398-5.475-8.226zM8.5 15.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm7 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3z" /></svg>
                  Download Ozzyl Lifestyle 
                </a>
              )}
            </div>
          </div>
