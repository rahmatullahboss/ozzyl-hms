...
export default function SettlementsPage({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data: rawData, isLoading: loading } = useApiQuery<any>(
    queryKeys.settlements.list(filters),
    `/api/settlements${qs ? `?${qs}` : ''}`,
  );
  const settlements: Settlement[] = rawData?.settlements ?? [];
  ...
  const createMutation = useApiMutation<any, any>('post', '/api/settlements', {
    onSuccess: () => {
    ...
    onError: (err) => { toast.error(err.message || t('settlementsPage.failed')); },
  });

  const markPaidMutation = useApiMutation<any, { id: number }>('put', (vars) => `/api/settlements/${vars.id}/pay`, {
    onSuccess: () => {
    ...
    <DashboardLayout role={role}>
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <KPICard title={t('settlementsPage.totalAmount')} value={`৳${(total ?? 0).toLocaleString()}`} loading={loading} icon={<Handshake className="w-5 h-5" />} iconBg="bg-[var(--color-primary-light)] text-[var(--color-primary)]" index={0} />
                ...
                {loading
                  ...
                  : settlements.map(s => (
                      <tr key={s.id}>
                        ...
                        <td className="font-data font-medium text-right">৳{(s.amount ?? 0).toLocaleString()}</td>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="btn-primary">{createMutation.isPending ? t('settlementsPage.saving') : t('settlementsPage.createSettlementBtn')}</button>
              </div>
            </form>
          </div>
        </div>
