...
export default function PrintTemplateSettings({ role }: { role?: string }) {
  ...
  const filterParams: Record<string, unknown> = typeFilter ? { type: typeFilter } : {};
  const apiPath = typeFilter
    ? `/api/print-templates?type=${encodeURIComponent(typeFilter)}`
    : '/api/print-templates';

  const { data: templatesData, isLoading: loading } = useApiQuery<{ data: Template[] }>(
    queryKeys.printTemplates.list(filterParams),
    apiPath,
  );
  ...
  const invalidateAll = () => {
    queryClient.invalidateQueries({ queryKey: queryKeys.printTemplates.all });
  };

  const createMutation = useApiMutation<unknown, typeof form>('post', '/api/print-templates', {
    onSuccess: () => {
      toast.success(t('settings.template_saved'));
      setShowForm(false);
      invalidateAll();
    },
    onError: () => toast.error(t('settings.failed')),
  });
  ...
  const testPrint = (id: number) => {
    window.open(`/api/print-templates/${id}/preview?testPrint=1`, '_blank');
  };

  const setDefault = async (id: number) => {
    try {
      await api.put(`/api/print-templates/${id}`, { is_default: true });
      toast.success(t('settings.set_as_default'));
      invalidateAll();
    } catch {
  ...
  const deleteTemplate = async (id: number) => {
    try {
      await api.delete(`/api/print-templates/${id}`);
          ...
          : templates.map(t => (
            <div key={t.id} className={`card p-5 ${t.is_default ? 'border-2 border-[var(--color-primary)]' : ''}`}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => testPrint(t.id)} className="btn-ghost text-xs"><Printer className="w-3.5 h-3.5" /> Test Print</button>
                {!t.is_default && <button onClick={() => setDefault(t.id)} className="btn-ghost text-xs"><Check className="w-3.5 h-3.5" /> Set Default</button>}
                <button onClick={() => deleteTemplate(t.id)} className="btn-ghost text-xs text-red-400" aria-label={`Delete ${t.template_name}`}><X className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input className="input w-full" value={form.logo_url} onChange={e => setForm({...form, logo_url: e.target.value})}
