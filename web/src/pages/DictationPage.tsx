...
import { Mic, Plus, X, Search, FileText } from 'lucide-react';
...
export default function DictationPage({ role }: { role?: string }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [listRes, statsRes] = await Promise.all([
        api.get<{ Results?: Dictation[] }>(`/api/dictation${qs}`),
        api.get<{ Results?: any }>('/api/dictation/stats'),
      ]);
      setDictations((listRes as any)?.Results || []);
      setStats((statsRes as any)?.Results || {});
    } catch {
      toast.error(t('failed_to_load_dictations'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedPatient) return toast.error(t('select_a_patient'));
    setSaving(true);
    try {
      await api.post('/api/dictation', {
      ...
      toast.success(t('dictation_created'));
      setShowNewModal(false);
      resetForm();
      queryClient.invalidateQueries({ queryKey: queryKeys.dictation.all });
      loadData();
    } catch {
      toast.error(t('failed_to_create_dictation'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleTranscribeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/api/dictation/${showTranscribeModal}/transcription`, {
        TranscriptionText: transcriptionText,
      });
      toast.success(t('transcription_saved'));
      setShowTranscribeModal(null);
      setTranscriptionText('');
      queryClient.invalidateQueries({ queryKey: queryKeys.dictation.all });
      loadData();
    } catch {
      toast.error(t('failed_to_save_transcription'));
    } finally {
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="page-title">{t('dictation.title')}</h1>
              <p className="text-sm text-[var(--color-text-muted)]">{t('dictation.manageMedicalRecordings')}</p>
            </div>
          </div>
          <button onClick={() => setShowNewModal(true)} className="btn btn-primary flex items-center gap-2">
            <Plus className="w-4 h-4" /> {t('dictation.newDictation')}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="card p-4 flex flex-col cursor-pointer hover:border-indigo-500 transition-colors" onClick={() => setStatusFilter('')}>
            <span className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">{t('dictation.all')}</span>
          ...
          <div className="card p-4 flex flex-col cursor-pointer hover:border-blue-500 transition-colors" onClick={() => setStatusFilter('pending')}>
            <span className="text-xs font-semibold text-blue-600 uppercase tracking-wider">{t('dictation.pending')}</span>
          ...
          <div className="card p-4 flex flex-col cursor-pointer hover:border-amber-500 transition-colors" onClick={() => setStatusFilter('in-progress')}>
            <span className="text-xs font-semibold text-amber-600 uppercase tracking-wider">{t('dictation.inProgress')}</span>
          ...
          <div className="card p-4 flex flex-col cursor-pointer hover:border-emerald-500 transition-colors" onClick={() => setStatusFilter('completed')}>
            <span className="text-xs font-semibold text-emerald-600 uppercase tracking-wider">{t('dictation.completed')}</span>
          ...
          <div className="card p-4 flex flex-col cursor-pointer hover:border-red-500 transition-colors" onClick={() => setStatusFilter('cancelled')}>
            <span className="text-xs font-semibold text-red-600 uppercase tracking-wider">{t('dictation.cancelled')}</span>
        ...
        <div className="card overflow-hidden">
          {loading ? (
             <div className="p-12 text-center text-[var(--color-text-muted)]">{t('common.loading', 'Loading...')}</div>
          ) : dictations.length === 0 ? (
             <div className="p-12 text-center flex flex-col items-center">
               <FileText className="w-12 h-12 text-[var(--color-text-muted)] opacity-20 mb-3" />
               <p className="text-[var(--color-text-muted)]">{t('dictation.noDictationsFound', 'No dictations found.')}</p>
             </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="table-base w-full">
                <thead>
                  <tr>
                    <th>{t('dictation.id')}</th>
                    <th>{t('dictation.date')}</th>
                    <th>{t('dictation.patientId')}</th>
                    <th>{t('dictation.priority')}</th>
                    <th>{t('dictation.status')}</th>
                    <th className="text-right">{t('dictation.actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {dictations.map(doc => (
                    <tr key={doc.DictationId}>
                      ...
                      <td className="font-data text-sm">{formatDisplayDate(doc.CreatedAt)}</td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-[var(--color-bg-primary)] rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
              <h3 className="font-bold text-[var(--color-text)]">{t('dictation.recordDictation')}</h3>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-5">
              ...
              <div>
                ...
                <select value={newPriority} onChange={e => setNewPriority(e.target.value as any)} className="input w-full">
                  <option value="normal">{t('common.priorityLevel.normal', 'Normal')}</option>
                  <option value="urgent">{t('common.priorityLevel.urgent', 'Urgent')}</option>
                  <option value="stat">{t('common.priorityLevel.stat', 'STAT')}</option>
                </select>
              </div>

              <div>
                <label className="label">{t('dictation.audiotextInput')}</label>
                <textarea
                  required
                  rows={4}
                  placeholder={t("dictation.mvpPlaceholder")}
                  value={newText}
              ...
              <div className="pt-4 flex justify-end gap-2">
                <button type="button" onClick={() => setShowNewModal(false)} className="btn btn-secondary">{t('common.cancel')}</button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-[var(--color-bg-primary)] rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)] bg-gray-50/50 dark:bg-gray-900/50">
              <h3 className="font-bold text-[var(--color-text)]">{t('dictation.transcribeDictation')}</h3>
            ...
            <form onSubmit={handleTranscribeSubmit} className="p-6 space-y-5">
              ...
              <div className="bg-[var(--color-bg-secondary)] p-4 rounded-lg border border-[var(--color-border)]">
                <h4 className="text-xs font-bold text-[var(--color-primary)] uppercase tracking-wider mb-2">{t('dictation.originalDictationText')}</h4>
              ...
              <div>
                <label className="label">{t('dictation.finalTranscriptionText')}</label>
                <textarea
                  required
                  rows={8}
                  placeholder={t("dictation.typeFullClinicalTranscription")}
              ...
              <div className="pt-4 flex justify-end gap-2">
                <button type="button" onClick={() => setShowTranscribeModal(null)} className="btn btn-secondary">{t('common.cancel')}</button>
                <button type="submit" disabled={transcribing} className="btn bg-indigo-600 text-white hover:bg-indigo-700 min-w-[120px]">
                  {transcribing ? t('common.completing', 'Completing...') : t('dictation.completeTranscription', 'Complete Transcription')}
                </button>
              </div>
            </form>
          </div>
