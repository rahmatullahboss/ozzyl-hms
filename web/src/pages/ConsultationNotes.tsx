...
export default function ConsultationNotes({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const listPath = statusFilter
    ? `/api/consultations?status=${encodeURIComponent(statusFilter)}`
    : '/api/consultations';

  const { data: listData, isLoading: loading } = useApiQuery<ConsultationListResponse>(
    queryKeys.consultations.list({ status: statusFilter }),
    listPath,
  );

  const consultations = listData?.consultations ?? [];

  const createMutation = useApiMutation<unknown, CreateConsultationPayload>(
    'post',
    '/api/consultations',
    {
      onSuccess: () => {
        toast.success(t('clinical.consultation_booked'));
        setShowCreate(false);
        setForm({ doctorId: '', patientId: '', scheduledAt: '', durationMin: '30', chiefComplaint: '', notes: '' });
  ...
  const todayCount = consultations.filter(c => c.scheduled_at.startsWith(gmt6Today)).length;
  ...
  const viewDetail = async (con: Consultation) => {
    try {
      const data = await api.get<ConsultationDetailResponse>(`/api/consultations/${con.id}`);
      setSelected(data.consultation ?? con);
    } catch {
      setSelected(con);
    }
    setShowDetail(true);
      ...
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        <div className="card p-4 flex flex-wrap gap-3 items-center">
          ...
          <div className="flex border border-[var(--color-border)] rounded-lg overflow-hidden text-sm">
            {[['', 'All'], ['waiting', 'Waiting'], ['scheduled', 'Scheduled'], ['in_progress', 'Active'], ['completed', 'Done'], ['referred', 'Referred'], ['follow_up_required', 'Follow-up'], ['cancelled', 'Cancelled']].map(([val, label]) => (
