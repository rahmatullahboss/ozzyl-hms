  ...
  const load = useCallback(async (pg = 1) => {
    ...
    try {
      const params = new URLSearchParams({
        page: String(pg), limit: String(LIMIT), from_date: from, to_date: to,
      });
      const data = await api.get<LedgerResponse>(
        `/api/pharmacy/suppliers/${supplierId}/ledger?${params.toString()}`,
      );
      ...
      if (e instanceof ApiClientError && e.status === 404) toast.error(t('pharmacy.supplier_not_found'));
      else toast.error(t('pharmacy.failed_to_load_ledger'));
    } finally { setLoading(false); }
  }, [supplierId, from, to, t]);
