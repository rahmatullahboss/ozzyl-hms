      ...
      <div className="modal max-w-md">
        ...
        <div className="modal-body space-y-4">
          ...
          <div className="modal-footer">
            <button className="btn btn-ghost" onClick={onCancel}>{t('cancel', { ns: 'common' })}</button>
            <button
              ...
              {t('confirm', { ns: 'common' })} {action === 'approve' ? t('approve') : t('reject')}
            </button>
          </div>
        </div>
      </div>
...
export default function ApprovalQueuePage({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data: grnData, isLoading: grnLoading, refetch: refetchGrn } = useApiQuery<{ data: PendingGRN[] }>(
    queryKeys.pharmacy.pendingGrnApprovals(),
    '/api/pharmacy/grn/pending-approval',
  );
  const { data: woData, isLoading: woLoading, refetch: refetchWo } = useApiQuery<{ data: PendingWriteOff[] }>(
    queryKeys.pharmacy.pendingWriteOffApprovals(),
    '/api/pharmacy/write-offs/pending-approval',
  );

  const grns = grnData?.data ?? [];
  const writeOffs = woData?.data ?? [];
  const loading = grnLoading || woLoading;
  ...
  const approveGrn = useApiMutation<unknown, { id: number; action: string; notes: string }>(
    'put',
    (vars) => `/api/pharmacy/grn/${vars.id}/approve`,
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.pendingGrnApprovals() });
        queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.goodsReceipts() });
      },
    },
  );

  const approveWriteOff = useApiMutation<unknown, { id: number; action: string; notes: string }>(
    'put',
    (vars) => `/api/pharmacy/write-offs/${vars.id}/approve`,
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.pendingWriteOffApprovals() });
      },
    },
  );
  ...
  const handleApprovalConfirm = async (action: 'approve' | 'reject', notes: string) => {
    ...
    try {
      if (dialog.type === 'grn') {
        await approveGrn.mutateAsync({ id: dialog.id, action, notes });
      } else {
        await approveWriteOff.mutateAsync({ id: dialog.id, action, notes });
      }
      ...
      <div className="space-y-5 max-w-screen-lg mx-auto">
        ...
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">{t('approvalQueueSubtitle')}</p>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={load}>
            <RefreshCw className="h-4 w-4 mr-1" /> {t('refresh', { ns: 'common' })}
          </button>
        </div>
