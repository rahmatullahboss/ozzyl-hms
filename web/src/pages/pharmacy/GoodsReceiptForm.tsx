...
export default function GoodsReceiptForm({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data: suppliersData } = useApiQuery<{ suppliers: Supplier[] }>(
    queryKeys.pharmacy.suppliers(),
    '/api/pharmacy/pharmacy-suppliers',
  );
  const { data: posData } = useApiQuery<{ purchaseOrders: PO[] }>(
    queryKeys.pharmacy.pendingPurchaseOrders(),
    '/api/pharmacy/purchase-orders?status=pending',
  );
  ...
  const createGRN = useApiMutation<unknown, unknown>(
    'post',
