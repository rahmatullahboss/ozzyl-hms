...
export default function CategoryList({ role = 'hospital_admin' }: { role?: string }) {
  ...
  // ── Query ────────────────────────────────────────────────────────────
  const { data, isLoading: loading } = useApiQuery<{ categories: Category[] }>(
  ...
  const updateMutation = useApiMutation<unknown, { id: number; name: string; description: string }>(
    'put',
    (vars) => `/api/pharmacy/categories/${vars.id}`,
    {
      onSuccess: () => {
  ...
  const toggleMutation = useApiMutation<unknown, Category>(
    'put',
    (cat) => `/api/pharmacy/categories/${cat.id}/${cat.is_active ? 'deactivate' : 'activate'}`,
    {
      onSuccess: (_data, cat) => {
        const action = cat.is_active ? 'deactivate' : 'activate';
        toast.success(t('pharmacy.categoryUpdated', { action }));
        queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.all });
      },
      onError: () => { toast.error(t('pharmacy.failed')); },
    },
  );
  ...
  const handleDeactivate = (cat: Category) => toggleMutation.mutate(cat);
