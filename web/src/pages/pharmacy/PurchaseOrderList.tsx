...
export default function PurchaseOrderList({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data, isLoading: loading } = useApiQuery<{ purchaseOrders: PO[] }>(
    queryKeys.pharmacy.purchaseOrders(filters),
    `/api/pharmacy/purchase-orders${statusFilter !== 'all' ? `?status=${statusFilter}` : ''}`,
  );
              ...
              <tbody>
                ...
                : pos.map(po => (
                  <tr key={po.id}>
                    ...
                    <td className="text-[var(--color-text-secondary)]">{formatDisplayDate(po.po_date)}</td>
