...
export default function PharmacyOverview({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const summaryQuery = useApiQuery<PharmacySummary>(
    queryKeys.pharmacy.summary(),
    '/api/pharmacy/summary',
  );

  const lowStockQuery = useApiQuery<AlertsResponse<LowStockAlert>>(
    queryKeys.pharmacy.lowStock(),
    '/api/pharmacy/alerts/low-stock',
  );

  const expiringQuery = useApiQuery<AlertsResponse<ExpiringAlert>>(
    queryKeys.pharmacy.expiring(),
    '/api/pharmacy/alerts/expiring?days=30',
  );

  const loading = summaryQuery.isLoading || lowStockQuery.isLoading || expiringQuery.isLoading;

  const summary: PharmacySummary = summaryQuery.data ?? {
    totalInvestment: 0, totalIncome: 0, totalCostOfGoodsSold: 0, grossProfit: 0,
  };
  const lowStock = (lowStockQuery.data?.alerts ?? []).slice(0, 8);
  const expiring = (expiringQuery.data?.alerts ?? []).slice(0, 8);
    ...
    <DashboardLayout role={role}>
      ...
      <div className="space-y-5 max-w-screen-2xl mx-auto">
        ...
        <div className="page-header">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to={`${base}/pharmacy/invoices/new`}>
              <button className="btn-primary"><Pill className="w-4 h-4 mr-1 inline" /> {t('newInvoice')}</button>
            </Link>
            <Link to={`${base}/pharmacy/po/new`}>
              <button className="btn-secondary text-sm"><ShoppingCart className="w-4 h-4 mr-1 inline" /> {t('createPO')}</button>
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard title={t('totalInvestment')} value={`৳${((summary.totalInvestment ?? 0) / 100).toLocaleString()}`} loading={loading} icon={<DollarSign className="w-5 h-5"/>} iconBg="bg-[var(--color-primary-light)] text-[var(--color-primary)]" />
          <KPICard title={t('totalIncome')} value={`৳${((summary.totalIncome ?? 0) / 100).toLocaleString()}`} loading={loading} icon={<Activity className="w-5 h-5"/>} iconBg="bg-emerald-50 text-emerald-600" />
          <KPICard title={t('grossProfit')} value={`৳${((summary.grossProfit ?? 0) / 100).toLocaleString()}`} loading={loading} icon={<TrendingDown className="w-5 h-5"/>} iconBg="bg-blue-50 text-blue-600" />
          <KPICard title={t('lowStockAlerts')} value={lowStock.length} loading={loading} icon={<AlertTriangle className="w-5 h-5"/>} iconBg="bg-amber-50 text-amber-600" />
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="table-base">
                <thead><tr>
                  ...
                  <th className="text-right">{t('reorderLevel')}</th>
                </tr></thead>
                <tbody>
                  {loading ? (
              ...
              <table className="table-base">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={4} className="py-12 text-center text-[var(--color-text-muted)]">✓ {t('noExpiring')}</td></tr>
                  ) : expiring.map(s => (
                    <tr key={s.id}>
                      <td className="font-medium">{s.item_name ?? '—'}</td>
                      <td className="text-[var(--color-text-secondary)] font-mono text-xs">{s.batch_no}</td>
                      <td className="text-[var(--color-text-secondary)] text-sm">{s.expiry_date ? formatDisplayDate(s.expiry_date) : '—'}</td>
