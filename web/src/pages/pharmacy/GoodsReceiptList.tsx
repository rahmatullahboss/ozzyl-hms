...
export default function GoodsReceiptList({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data, isLoading: loading } = useApiQuery<{ goodsReceipts: GRN[] }>(
    queryKeys.pharmacy.goodsReceipts(),
    '/api/pharmacy/goods-receipts',
  );

  const grns = data?.goodsReceipts ?? [];
              ...
              <tbody>
                ...
                : grns.map(g => (
                  <tr key={g.id}>
                    ...
                    <td className="text-[var(--color-text-secondary)]">{formatDisplayDate(g.received_date)}</td>
