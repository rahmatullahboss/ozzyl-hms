...
export default function ItemPriceHistory({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const load = useCallback(async (id: string) => {
    ...
    setLoading(true);
    try {
      const data = await api.get<{ data: PriceEntry[] }>(`/api/pharmacy/items/${id}/price-history`);
      setEntries(data.data ?? []);
      if (data.data?.length > 0) {
        setItemName(`Item #${id}`);
      }
    } catch (e) {
      if (e instanceof ApiClientError && e.status === 404) toast.error(t('priceHistory.itemNotFound', { defaultValue: 'Item not found' }));
      else toast.error(t('priceHistory.failedLoad', { defaultValue: 'Failed to load price history' }));
    } finally { setLoading(false); }
  }, [t]);

  const recordPriceMutation = useMutation<unknown, Error, {
    itemId: string;
    body: Record<string, unknown>;
  }>({
    mutationFn: ({ itemId: id, body }) => api.post(`/api/pharmacy/items/${id}/price-history`, body),
    onSuccess: () => {
    ...
    onError: () => { toast.error(t('priceHistory.failedRecord', { defaultValue: 'Failed to record price change' })); },
  });
  ...
  const handleRecordPrice = (e: React.FormEvent) => {
    e.preventDefault();
          ...
          <div className="card p-4">
            ...
            <form onSubmit={handleRecordPrice} className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="col-span-2 md:col-span-4 flex gap-2">
                <button type="submit" className="btn btn-primary btn-sm" disabled={recordPriceMutation.isPending}>
                  {recordPriceMutation.isPending && <Loader2 className="h-4 w-4 animate-spin mr-1" />}
