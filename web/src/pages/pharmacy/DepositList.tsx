...
export default function DepositList({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data, isLoading: loading } = useApiQuery<{ deposits: Deposit[] }>(
    queryKeys.pharmacy.deposits(),
    '/api/pharmacy/deposits',
  );
  const deposits = data?.deposits ?? [];

  const createMutation = useApiMutation<unknown, {
    patientId: number; amount: number; paymentMode: 'cash' | 'card' | 'mobile'; remarks?: string;
  }>('post', '/api/pharmacy/deposits', {
    onSuccess: () => {
      toast.success(t('depositRecorded', { defaultValue: 'Deposit recorded' }));
      queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.deposits() });
      setShowModal(false);
    },
    onError: (err) => {
      toast.error(err.message || t('pharmacy.operationFailed'));
    },
  });
              ...
              <tbody>
                ...
                : deposits.map(d => (
                  <tr key={d.id}>
                    ...
                    <td>{d.created_at ? formatDisplayDate(d.created_at) : '—'}</td>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal w-full max-w-md">
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button type="submit" disabled={createMutation.isPending} className="btn-primary">{createMutation.isPending ? '…' : t('record', { defaultValue: 'Record' })}</button>
                </div>
              </form>
            </div>
          </div>
