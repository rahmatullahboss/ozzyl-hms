...
export default function ItemList({ role = 'hospital_admin' }: { role?: string }) {
  ...
  // ── Queries ──────────────────────────────────────────────────────────
  const { data: itemsData, isLoading: loading } = useApiQuery<{ items: Item[] }>(
    queryKeys.pharmacy.items(search || undefined),
    `/api/pharmacy/items${search ? `?search=${encodeURIComponent(search)}` : ''}`,
  );
  const items = itemsData?.items ?? [];

  // Dropdown queries — only fetched when modal is open
  const { data: catData } = useApiQuery<{ categories: Category[] }>(
    queryKeys.pharmacy.categories(),
    '/api/pharmacy/categories',
    { enabled: showModal },
  );
  const { data: genData } = useApiQuery<{ generics: Generic[] }>(
    queryKeys.pharmacy.generics(),
    '/api/pharmacy/generics',
    { enabled: showModal },
  );
  const { data: uomData } = useApiQuery<{ uoms: UOM[] }>(
    queryKeys.pharmacy.uom(),
    '/api/pharmacy/uom',
    { enabled: showModal },
  );
  const { data: ptData } = useApiQuery<{ packingTypes: PackingType[] }>(
    queryKeys.pharmacy.packingTypes(),
    '/api/pharmacy/packing-types',
    { enabled: showModal },
  );
  const categories = catData?.categories ?? [];
  const generics = genData?.generics ?? [];
  const uoms = uomData?.uoms ?? [];
  const packingTypes = ptData?.packingTypes ?? [];

  // ── Mutations ────────────────────────────────────────────────────────
  const toggleMutation = useApiMutation<unknown, Item>(
    'put',
    (item) => `/api/pharmacy/items/${item.id}/${item.is_active ? 'deactivate' : 'activate'}`,
    {
      ...
      onError: () => { toast.error(t('failedStatus', { defaultValue: 'Failed to update item status' })); },
    },
  );

  const saveMutation = useApiMutation<unknown, { payload: Record<string, unknown>; id?: number }>(
    'post',
    (vars) => vars.id ? `/api/pharmacy/items/${vars.id}` : '/api/pharmacy/items',
    {
      onSuccess: (_data, vars) => {
        toast.success(vars.id ? t('itemUpdated', { defaultValue: 'Item updated' }) : t('itemCreated', { defaultValue: 'Item created' }));
        setShowModal(false);
        queryClient.invalidateQueries({ queryKey: queryKeys.pharmacy.all });
      },
      onError: (err) => {
        const msg = err instanceof ApiClientError ? err.message : t('item.failedSave', { defaultValue: 'Failed to save item' });
        toast.error(msg || t('item.failed', { defaultValue: 'Failed' }));
      },
    },
  );

  // We need to override method for edits (PUT vs POST)
  const updateMutation = useApiMutation<unknown, { payload: Record<string, unknown>; id: number }>(
    'put',
    (vars) => `/api/pharmacy/items/${vars.id}`,
    {
      onSuccess: () => {
  ...
  const handleToggle = (item: Item) => toggleMutation.mutate(item);
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div className="flex gap-1.5">
                        <button onClick={() => openEdit(item)} className="btn-ghost p-1.5" title={t('edit', { ns: 'common' })}><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => handleToggle(item)} className="btn-ghost p-1.5" title={item.is_active ? t('deactivate') : t('activate')}>{item.is_active ? <ToggleRight className="w-4 h-4 text-emerald-600" /> : <ToggleLeft className="w-4 h-4 text-[var(--color-text-muted)]" />}</button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4">
                {!editing && (
                  <div className="bg-[var(--color-bg-soft)] rounded-xl p-3 border border-dashed border-[var(--color-border)]">
                    <label className="label text-xs mb-1.5">🔍 {t('searchMasterDB')}</label>
                ...
                <div><label className="label">{t('itemName')} *</label><input className="input" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder={t('item.namePlaceholder')} /></div>
