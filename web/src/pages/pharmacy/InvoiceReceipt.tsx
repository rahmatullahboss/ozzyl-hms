...
export default function InvoiceReceipt({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data, isLoading: loading } = useApiQuery<{ invoice: Invoice; items: InvoiceItem[] }>(
    queryKeys.pharmacy.invoiceReceipt(id ?? ''),
    `/api/pharmacy/invoices/${id}/receipt`,
    { enabled: !!id },
  );
  ...
  const printCountMutation = useApiMutation<unknown, unknown>(
    'put',
    `/api/pharmacy/invoices/${id}/print-count`,
  );

  const handlePrint = async () => {
    // Increment print count on server
    try {
      printCountMutation.mutate({});
    } catch { /* non-blocking */ }
    window.print();
  };
