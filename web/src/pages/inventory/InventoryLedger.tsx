...
export default function InventoryLedger({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const { data: ledgerData, isLoading: loading } = useApiQuery<LedgerResponse>(
    queryKeys.inventoryLedger.transactions({ page, search, type: typeFilter }),
    `/api/inventory/stock/transactions?${params}`,
  );

  const transactions = ledgerData?.data ?? [];
  const total = ledgerData?.pagination?.total ?? 0;
              ...
              <tbody>
                ...
                transactions.map(tx => (
                  <tr key={tx.TransactionId}>
                    <td className="text-sm text-[var(--color-text-secondary)] whitespace-nowrap">{tx.TransactionDate ? formatDisplayDate(tx.TransactionDate) : '—'}</td>
