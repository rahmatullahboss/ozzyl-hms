...
export default function StockAdjustment({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const adjustStock = useApiMutation<unknown, unknown>(
    'post',
    '/api/inventory/stock/adjustments',
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: queryKeys.inventory.stock() });
        queryClient.invalidateQueries({ queryKey: queryKeys.inventory.stockAdjustments() });
        toast.success(t('inventory.stock_adjusted'));
        navigate(`${base}/inventory/stock`);
      },
      onError: (err) => {
        toast.error(err.message || t('inventory.adjustStockFailed'));
      },
    },
  );
  ...
  const saving = adjustStock.isPending;
