...
function OrdersTab() {
  ...
  const buildOrdersPath = () => {
    ...
    return `/api/kitchen/orders?${params.toString()}`;
  };

  const { data: ordersData, isLoading: loading } = useApiQuery<{ data?: MealOrder[] }>(
    queryKeys.kitchen.orders({ date, meal: mealFilter, status: statusFilter }),
    buildOrdersPath(),
    { placeholderData: (prev: { data?: MealOrder[] } | undefined) => prev },
  );
  const orders = ordersData?.data ?? [];

  const { data: statsData } = useApiQuery<{ stats?: Stats }>(
    queryKeys.kitchen.stats(date),
    `/api/kitchen/stats?date=${date}`,
  );
  const stats = statsData?.stats ?? null;

  const updateStatusMutation = useApiMutation<unknown, { id: number; status: string }>(
    'put',
    (vars) => `/api/kitchen/orders/${vars.id}/status`,
    {
      onSuccess: (_data, vars) => {
        toast.success(t('orderStatusUpdated', { status: vars.status }));
  ...
  const generateMutation = useApiMutation<{ generated: number }, { order_date: string; meal_name: string }>(
    'post',
    '/api/kitchen/orders/generate',
    {
      onSuccess: (data) => {
  ...
  const refetch = () => {
    queryClient.invalidateQueries({ queryKey: queryKeys.kitchen.orders({ date, meal: mealFilter, status: statusFilter }) });
    queryClient.invalidateQueries({ queryKey: queryKeys.kitchen.stats(date) });
  };
                ...
                <tr key={o.id}>
                  ...
                  <td>
                    <div className="flex gap-1">
                      {o.status === 'pending' && <button onClick={() => updateStatusMutation.mutate({ id: o.id, status: 'preparing' })} className="btn-ghost text-xs p-1" title={t('preparing')}><ChefHat className="w-3.5 h-3.5" /></button>}
                      {o.status === 'preparing' && <button onClick={() => updateStatusMutation.mutate({ id: o.id, status: 'ready' })} className="btn-ghost text-xs p-1 text-blue-600" title={t('ready')}><CheckCircle className="w-3.5 h-3.5" /></button>}
                      {o.status === 'ready' && <button onClick={() => updateStatusMutation.mutate({ id: o.id, status: 'delivered' })} className="btn-ghost text-xs p-1 text-green-600" title={t('delivered')}><Truck className="w-3.5 h-3.5" /></button>}
                      {o.status !== 'delivered' && o.status !== 'cancelled' && <button onClick={() => updateStatusMutation.mutate({ id: o.id, status: 'cancelled' })} className="btn-ghost text-xs p-1 text-red-400" title={t('cancelled')}><X className="w-3.5 h-3.5" /></button>}
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showGenerate && (
        <Modal title={t('generateMealOrders')} onClose={() => setShowGenerate(false)}>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={() => generateMutation.mutate({ order_date: date, meal_name: genMeal })} disabled={generateMutation.isPending} className="btn-primary">{generateMutation.isPending ? t('generating') : t('generate')}</button>
          </div>
        </Modal>
      )}
    </div>
...
function WardViewTab() {
  ...
  const { data: rowsData, isLoading: loading } = useApiQuery<{ data?: WardRow[] }>(
    queryKeys.kitchen.wardSummary(date),
    `/api/kitchen/ward-summary?date=${date}`,
  );
...
function DietTypesTab() {
  ...
  const { data: itemsData, isLoading: loading } = useApiQuery<{ data?: DietType[] }>(
    queryKeys.kitchen.dietTypes(),
    '/api/kitchen/diet-types',
  );
  ...
  const createMutation = useApiMutation<unknown, typeof form>(
    ...
    {
      onSuccess: () => {
        toast.success(t('dietTypeAdded'));
        setShowForm(false);
        setForm({ diet_name: '', description: '', calories_range: '', restrictions: '' });
        queryClient.invalidateQueries({ queryKey: queryKeys.kitchen.dietTypes() });
      },
      onError: () => { toast.error(t('failed')); },
    },
  );
  ...
  const deleteMutation = useApiMutation<unknown, { id: number }>(
    'delete',
    (vars) => `/api/kitchen/diet-types/${vars.id}`,
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: queryKeys.kitchen.dietTypes() });
      },
            ...
            <tbody>
              ...
              : items.map(d => (
                <tr key={d.id}>
                  ...
                  <td><button onClick={() => deleteMutation.mutate({ id: d.id })} className="btn-ghost text-xs text-red-400 p-1"><X className="w-3.5 h-3.5" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Modal title={t('addDietTypeTitle')} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" disabled={createMutation.isPending} className="btn-primary">{createMutation.isPending ? t('saving') : t('add')}</button>
            </div>
          </form>
        </Modal>
      )}
...
function SchedulesTab() {
  ...
  const { data: itemsData, isLoading: loading } = useApiQuery<{ data?: MealSchedule[] }>(
    queryKeys.kitchen.mealSchedules(),
    '/api/kitchen/meal-schedules',
  );
  const items = itemsData?.data ?? [];
  ...
  const createMutation = useApiMutation<unknown, typeof form>(
    ...
    '/api/kitchen/meal-schedules',
    {
      onSuccess: () => {
        ...
        setShowForm(false);
        queryClient.invalidateQueries({ queryKey: queryKeys.kitchen.mealSchedules() });
      },
      onError: () => { toast.error(t('failed')); },
    },
  );
