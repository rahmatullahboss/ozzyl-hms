...
export default function GroupAttendance({ role }: { role?: string }) {
  ...
  const { data: sessionsData, isLoading: loadingSessions } = useApiQuery<{ Results: Session[] }>(
    queryKeys.groupAttendance.sessions(),
    '/api/group-attendance'
  );
  const sessions = sessionsData?.Results ?? [];

  // --- Create session mutation ---
  const createSessionMutation = useApiMutation<unknown, Record<string, unknown>>(
    'post',
    '/api/group-attendance',
    {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: queryKeys.groupAttendance.all });
      },
      onError: () => { toast.error(t('groupAttendance.createSessionFailed')); },
    }
  );

  // --- Save attendance mutation ---
  const saveAttendanceMutation = useApiMutation<unknown, { sessionId: number; PatientId: number; Status: string; MoodRating?: number; ParticipationLevel?: string }>(
    'post',
    (vars) => `/api/group-attendance/${vars.sessionId}/attendance`,
    {
      onSuccess: () => { toast.success(t('groupAttendance.attendanceSaved')); },
      onError: () => { toast.error(t('groupAttendance.saveAttendanceFailed')); },
    }
  );

  // --- Fetch session detail (imperative because depends on selection) ---
  const fetchSessionDetail = async (session: Session) => {
    ...
    try {
      const [membersRes, attendanceRes] = await Promise.all([
        fetch(`/api/group-attendance/${session.SessionId}/members`, {
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAccessToken() ?? ''}` },
        }).then(r => r.json()),
        fetch(`/api/group-attendance/${session.SessionId}/attendance`, {
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAccessToken() ?? ''}` },
        }).then(r => r.json()),
      ]);

      const memberList: Member[] = membersRes?.Results ?? membersRes ?? [];
      const attendanceList: AttendanceRecord[] = attendanceRes?.Results ?? attendanceRes ?? [];

      const formMap: Record<number, AttendanceForm> = {};
      const existingById: Record<number, AttendanceRecord> = {};
      attendanceList.forEach((a) => { existingById[a.PatientId] = a; });
