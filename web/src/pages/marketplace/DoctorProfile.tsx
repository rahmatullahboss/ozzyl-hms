...
export default function DoctorProfile() {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setBookMsg(res.ok ? t('marketing:profile.bookingSuccess') : (data.error || t('marketing:profile.bookingFailed')));
    } catch {
      setBookMsg(t('marketing:profile.bookingFailed'));
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center gap-2 text-sm text-gray-500">
          <Link to="/marketplace" className="hover:text-blue-600">{t('marketing:directory.doctors.marketplace') || 'Marketplace'}</Link>
          <span>/</span>
          <Link to="/marketplace/doctors" className="hover:text-blue-600">{t('marketing:directory.doctors.title')}</Link>
          <span>/</span>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4 text-sm">
                <div className="bg-gray-50 rounded-lg p-2.5">
                  <p className="text-gray-500 text-xs">{t('marketing:cards.feeUnit')}</p>
                  <p className="font-semibold text-green-700">{feeDisplay}</p>
                </div>
                {doctor.bmdc_reg_no && (
                  <div className="bg-gray-50 rounded-lg p-2.5">
                    <p className="text-gray-500 text-xs">{t('marketing:profile.bmdcReg')}</p>
              ...
              {languages.length > 0 && (
                <p className="mt-2 text-xs text-gray-500">
                  {t('marketing:filters.language')}: {languages.map(l => t(`marketing:languages.${l}`)).join(', ')}
                </p>
              )}
            ...
            {schedule.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <h2 className="font-semibold text-gray-900 mb-3">{t('marketing:profile.schedule')}</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {schedule.map((s: any) => (
                    <div key={s.id} className="text-sm bg-green-50 rounded-lg p-2">
                      ...
                      <p className="text-xs text-gray-400">{t('marketing:profile.maxPatients', { count: s.max_patients })}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            <div className="bg-white rounded-xl border border-gray-200 p-4 sticky top-4">
              <h2 className="font-semibold text-gray-900 mb-3">{t('marketing:profile.bookAppointment')}</h2>

              <div className="space-y-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">{t('marketing:profile.date')}</label>
                  <input
                    type="date"
                    value={bookDate}
                ...
                {bookDate && availability.length > 0 && (
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">{t('marketing:profile.availableTimes')}</label>
                ...
                {bookDate && availability.length === 0 && (
                  <p className="text-xs text-gray-400 text-center py-2">{t('marketing:profile.noSlots')}</p>
                )}
                ...
                >
                  {booking ? t('marketing:profile.booking') : t('marketing:profile.confirmBooking')}
                </button>

                <p className="text-xs text-gray-400 text-center">
                  {feeDisplay} {t('marketing:cards.feeUnit')}
                </p>
