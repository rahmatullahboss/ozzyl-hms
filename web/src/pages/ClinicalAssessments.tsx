...
import {
  Brain, ListChecks, Stethoscope, BookOpen, UtensilsCrossed, Droplet, Search, Shield, Clipboard
} from 'lucide-react';
...
import SDOHTab from '../components/clinical/SDOHTab';
import ROSTab from '../components/clinical/ROSTab';
...
export default function ClinicalAssessments({ role = 'doctor' }: { role?: string }) {
  ...
  const TABS = [
    ...
    { key: 'sdoh', label: t('tabs.sdoh', 'SDOH'), icon: <Shield className="w-4 h-4" /> },
    { key: 'ros', label: t('tabs.ros', 'ROS'), icon: <Clipboard className="w-4 h-4" /> },
  ];
        ...
        {patientId ? (
          <div className="card flex flex-col min-h-[600px]">
            ...
            <div className="p-5 flex-1">
              ...
              {activeTab === 'sdoh' && <SDOHTab patientId={patientId} />}
              {activeTab === 'ros' && <ROSTab patientId={patientId} />}
            </div>
          </div>
        ) : (
