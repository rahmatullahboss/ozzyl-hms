...
export default function FeeSheet({ role }: { role?: string }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      api
        .get<{ patients?: Patient[] }>(`/api/patients?search=${encodeURIComponent(patientQuery)}&limit=10`)
        .then(r => setPatients((r as any).patients ?? []))
        .catch(() => setPatients([]));
    }, 300);
  ...
  const loadTransactions = useCallback(async (patientId: number) => {
    setTxLoading(true);
    try {
      const r = await api.get<{ transactions?: Transaction[] }>(`/api/fee-sheet?patientId=${patientId}`);
      setTransactions((r as any).transactions ?? (r as unknown as Transaction[]) ?? []);
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowForm(false);
      resetForm();
      queryClient.invalidateQueries({ queryKey: queryKeys.feeSheet.list(selectedPatient.id) });
  ...
  const handleStatusUpdate = async (txId: number, status: Transaction['BillingStatus']) => {
    setStatusMenuId(null);
    try {
      await api.put(`/api/fee-sheet/${txId}/status`, { Status: status });
                      ...
                      return (
                        <tr key={tx.TransactionId} className="hover:bg-[var(--color-bg-secondary)] transition-colors">
                          ...
                          <td className="px-4 py-3 flex items-center gap-1.5 text-[var(--color-text)]">
                            ...
                            {formatDisplayDate(tx.VisitDate)}
                          </td>
