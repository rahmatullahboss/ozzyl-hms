...
import { Bot, Send, ThumbsUp, ThumbsDown, Stethoscope, Pill, FileText, Activity, Brain, BarChart3 } from 'lucide-react';
...
export default function AIAssistant({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const sendFeedback = async (action: 'accepted' | 'rejected') => {
    ...
    try {
      await api.post('/api/ai/feedback', { interactionId, action });
  ...
  const submit = async () => {
    ...
    try {
      const data = await api.post<Record<string, unknown>>(feat.endpoint, payload);
      setResult(data);
      setInteractionId((data.interactionId as number) ?? null);
    } catch (err) {
