...
export default function TestCatalog({ role = 'hospital_admin' }: { role?: string }) {
  ...
  const queryPath = statusFilter ? `/api/tests?status=${statusFilter}` : '/api/tests';

  const { data, isLoading: loading } = useApiQuery<TestsResponse>(
    queryKeys.laboratory.list({ status: statusFilter }),
    queryPath,
  );
  ...
  const createMutation = useApiMutation<TestsResponse, { patientId: number; testName: string }>(
    'post',
    '/api/tests',
    {
      onSuccess: () => {
        toast.success(t('laboratory.testOrdered'));
        setShowCreate(false);
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    createMutation.mutate({ patientId: parseInt(form.patientId), testName: form.testName });
  };
