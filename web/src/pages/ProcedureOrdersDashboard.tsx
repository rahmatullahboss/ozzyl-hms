...
interface ProcedureOrdersResponse {
  Results: ProcedureOrder[];
}
...
interface CreateOrderPayload {
  ...
  ProcedureOrderType: string;
  OrderDiagnosis: string;
  ProcedureCodes: { ProcedureCode: string; ProcedureName: string }[];
}

export default function ProcedureOrdersDashboard({ role }: { role?: string }) {
  const { t } = useTranslation('procedure-orders');
  const queryClient = useQueryClient();

  // ── Query: load procedure orders ──────────────────────────────────────
  const { data, isLoading: loading } = useApiQuery<ProcedureOrdersResponse>(
    queryKeys.procedureOrders.list(),
    '/api/procedure-orders?limit=100',
  );
  const orders = data?.Results ?? [];

  // ── Mutation: create procedure order ──────────────────────────────────
  const createOrder = useApiMutation<unknown, CreateOrderPayload>(
    'post',
    '/api/procedure-orders',
    {
      onSuccess: () => {
        toast.success(t('notifications.created'));
        queryClient.invalidateQueries({ queryKey: queryKeys.procedureOrders.all });
        setShowModal(false);
        setNewOrder(prev => ({ ...prev, ProcedureCode: '', ProcedureName: '', OrderDiagnosis: '' }));
      },
      onError: (err) => {
  ...
  const handleSaveOrder = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newOrder.PatientId || !newOrder.ProcedureCode) return toast.error(t('notifications.fillRequired'));
    ...
    <DashboardLayout role={role ?? 'hospital_admin'}>
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
        ...
        <div className="card overflow-hidden">
          {loading ? (
             <div className="p-8 text-center text-sm text-[var(--color-text-muted)]">{t('loading')}</div>
          ) : orders.length === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-[var(--color-text-muted)]">
              <FileText className="w-10 h-10 mb-3 opacity-30" />
              <p className="text-sm font-medium mb-4">{t('noOrders')}</p>
              ...
              <tbody>
                {orders.map(o => (
                  <tr key={o.ProcedureOrderId}>
                    ...
                    <td className="font-data">{formatDisplayDate(o.DateOrdered)}</td>
          ...
          <div className="bg-[var(--color-card)] rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-[var(--color-text)]">{t('modal.title')}</h3>
                </div>
              </div>
              <button type="button" onClick={() => setShowModal(false)} className="btn btn-secondary p-1.5 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveOrder} className="p-6 space-y-4">
               <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label">{t('modal.patientId')}</label>
                  <input
                    required
                    type="number"
                ...
                <div>
                  <label className="label">{t('modal.providerId')}</label>
                  <input
                    required
                    type="number"
                    className="input w-full"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label">{t('modal.orderType')}</label>
                  <select
                    className="input w-full"
                    value={newOrder.ProcedureOrderType}
                    onChange={e => setNewOrder(p => ({...p, ProcedureOrderType: e.target.value}))}
                  >
                    <option value="laboratory_test">{t('types.laboratory_test')}</option>
                    <option value="radiology">{t('types.radiology')}</option>
                    <option value="cardiology">{t('types.cardiology')}</option>
                    <option value="referral">{t('types.referral')}</option>
                    <option value="other">{t('types.other')}</option>
                  </select>
                </div>
                <div>
                  <label className="label">{t('modal.priority')}</label>
                  <select
                    className="input w-full"
                    value={newOrder.OrderPriority}
                    onChange={e => setNewOrder(p => ({...p, OrderPriority: e.target.value}))}
                  >
                    <option value="routine">{t('priorities.routine')}</option>
                    <option value="stat">{t('priorities.stat')}</option>
                    <option value="urgent">{t('priorities.urgent')}</option>
                    <option value="asap">{t('priorities.asap')}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="col-span-1">
                  <label className="label">{t('modal.procedureCode')}</label>
                  <input
                    required
                    className="input w-full"
                    placeholder="e.g. 1"
                ...
                <div className="col-span-2">
                  <label className="label">{t('modal.procedureName')}</label>
                  <input
                    required
                    className="input w-full"
              ...
              <div>
                <label className="label">{t('modal.diagnosis')}</label>
                <input
                  className="input w-full"
                  placeholder="..."
              ...
              <div className="flex gap-2 pt-4 border-t border-[var(--color-border)] justify-end">
                <button type="button" onClick={() => setShowModal(false)} className="btn btn-secondary">{t('modal.cancel')}</button>
                <button type="submit" disabled={createOrder.isPending} className="btn btn-primary min-w-[120px]">
                  {createOrder.isPending ? t('modal.creating') : t('modal.create')}
                </button>
              </div>
            </form>
          </div>
