...
import { ShieldAlert, Plus, X, Activity, FileText } from 'lucide-react';
...
export default function PriorAuthDashboard({ role }: { role?: string }) {
  ...
  const { data: authsData, isLoading: loading } = useApiQuery<{ Results?: PriorAuth[] }>(
    queryKeys.priorAuth.list(),
    '/api/prior-auth?limit=100',
  );
  const auths = authsData?.Results ?? [];
  ...
  const createMutation = useApiMutation<unknown, Record<string, unknown>>(
    ...
    '/api/prior-auth',
    {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: queryKeys.priorAuth.all });
      },
      onError: (err) => {
        toast.error(err.message || t('billing.failed_to_submit_request'));
      },
    },
  );
      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
        <div className="page-header flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="page-title">{t('priorAuthorizations', { ns: 'billing' })}</h1>
              <p className="text-sm text-[var(--color-text-muted)]">{t('priorAuthSubtitle', { ns: 'billing' })}</p>
            </div>
          </div>
          <button onClick={() => setShowModal(true)} className="btn btn-primary flex items-center gap-2">
            <Plus className="w-4 h-4" /> {t('newRequest', { ns: 'billing' })}
          </button>
        </div>
          ...
          {loading ? (
            ...
            <div className="p-12 flex flex-col items-center justify-center text-[var(--color-text-muted)]">
              ...
              <button onClick={() => setShowModal(true)} className="btn btn-secondary flex items-center gap-2">
                <Plus className="w-4 h-4" /> {t('createFirstRequest', { ns: 'billing' })}
              </button>
            </div>
          ) : (
            <table className="table-base w-full text-sm">
              <thead>
                <tr>
                  <th>{t('date', { ns: 'billing' })}</th>
                  <th>{t('patient', { ns: 'billing' })}</th>
                  <th>{t('type', { ns: 'billing' })}</th>
                  <th>{t('service', { ns: 'billing' })}</th>
                  <th>{t('priority', { ns: 'billing' })}</th>
                  <th>{t('status', { ns: 'common' })}</th>
                  <th>{t('action', { ns: 'billing' })}</th>
                </tr>
              </thead>
              <tbody>
                {auths.map(pa => (
                  <tr key={pa.AuthId}>
                    <td className="font-data">{formatDisplayDate(pa.RequestDate)}</td>
