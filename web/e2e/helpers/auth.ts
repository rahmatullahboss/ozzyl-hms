...
export const roleUsers: Record<HMSRole, object> = {
  ...
  doctor: { id: 108, name: 'Dr. Aminul Islam', email: 'doctor@demo-hospital.com', role: 'doctor' },
};

/**
 * Log in as a given role through the real login form while mocking only the
 * authentication API. Staff access tokens are held in the application's
 * in-memory token store and intentionally never persist in localStorage.
 */
export async function loginAs(
  ...
  const user = roleUsers[role] as { id: number; name: string; email: string; role: string };
  ...
  await page.route(
    /https:\/\/(?:fonts\.googleapis\.com|fonts\.gstatic\.com|www\.googletagmanager\.com|www\.google-analytics\.com)\/.*/,
    (route: Route) => route.fulfill({ status: 204, body: '' }),
  );

  await page.route('**/api/auth/login-direct', (route: Route) => {
    route.fulfill({
      status: 200,
      ...
      body: JSON.stringify({
        ...
        slug: SLUG,
        user,
        hospital: { id: 1, name: 'Demo Hospital', slug: SLUG },
      }),
    });
  });
  ...
  await page.getByLabel(/email or mobile/i).fill(user.email);
