-- Unified visit-services layer (Danphe-style reception workflow)
-- Links all billable services to a visit for one-click billing
...
-- Procedure orders table (for suture removal, dressing, injection, etc.)
-- These are clinically performed but ordered from reception
...
-- Link existing lab_orders to visit_services via trigger-like update
-- (Run manually after deployment to backfill)

-- Add a bill_source column to payments for payment-method reporting
...
-- Add counter_id to bills for shift-wise reporting
