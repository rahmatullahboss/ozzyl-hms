...
export interface ClinicalDocumentDiagnosisProviderCoverage {
  ...
  provider: {
    ...
    narrativeSimilarityForbidden: boolean;
...
export interface ClinicalDocumentDiagnosisReadinessEvidence {
  schemaVersion: number;
  domain: string;
  checkpoint: string;
  claims: {
...
export interface ClinicalDocumentDiagnosisReadinessResult {
  localReady: boolean;
  productionReady: boolean;
  issues: string[];
  issueCount: number;
...
const COVERAGE_PATH = 'docs/database/canonical-clinical-document-diagnosis-provider-coverage.json';
const READINESS_PATH = 'docs/database/clinical-document-diagnosis-readiness.json';
const PROVIDER_IMPORT_PATTERN = /clinical-document-diagnosis-(?:provider|read-adapters)/;
...
export function validateClinicalDocumentDiagnosisReadiness(
  root: string,
  readiness: ClinicalDocumentDiagnosisReadinessEvidence,
  coverage: ClinicalDocumentDiagnosisProviderCoverage,
): string[] {
  const issues: string[] = [];
  if (coverage.schemaVersion !== 1 || readiness.schemaVersion !== 1) issues.push('schema version must be 1');
  if (coverage.domain !== 'clinical_document_diagnosis' || readiness.domain !== 'clinical_document_diagnosis') {
    issues.push('domain must be clinical_document_diagnosis');
  }
  if (readiness.checkpoint !== 'CDB-122E') issues.push('checkpoint must be CDB-122E');
  if (coverage.provider.flagKey !== 'canonical_clinical_document_diagnosis_provider_v1') {
  ...
  if (!coverage.provider.exactSourceMappingRequired || !coverage.provider.narrativeSimilarityForbidden) {
    issues.push('provider must require exact mapping and forbid narrative similarity');
  }
  ...
  for (const reader of coverage.knownReaders) {
    ...
    if (PROVIDER_IMPORT_PATTERN.test(source)) issues.push(`runtime reader activates clinical provider: ${reader.path}`);
  }
...
export function checkClinicalDocumentDiagnosisReadiness(root: string): ClinicalDocumentDiagnosisReadinessResult {
  const coverage = readJson<ClinicalDocumentDiagnosisProviderCoverage>(root, COVERAGE_PATH);
  const readiness = readJson<ClinicalDocumentDiagnosisReadinessEvidence>(root, READINESS_PATH);
  const issues = validateClinicalDocumentDiagnosisReadiness(root, readiness, coverage);
  return {
